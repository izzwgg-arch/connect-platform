...
test("AUTO-APPROVE DISABLED: owner-requested pbx.M1 still goes to PENDING_APPROVAL", async () => {
  const a = await svc().create({ tenantId: "21", capabilityId: "pbx.M1", params: P, summary: "MOH switch", requestedBy: "izzy", requestedRole: "owner", autoApprove: true });
  assert.equal(a.status, "PENDING_APPROVAL");
  assert.equal(a.paramsHash, computeParamsHash("pbx.M1", "21", P));
...
test("PENDING CAP: 4th concurrent modify request for a tenant is denied", async () => {
  const s = svc();
  for (let i = 0; i < 3; i++) {
    await s.create({ tenantId: "21", capabilityId: "pbx.M1", params: { ...P, classId: `c${i}` }, summary: `m${i}`, requestedBy: "u", requestedRole: "customer" });
  }
  const fourth = await s.create({ tenantId: "21", capabilityId: "pbx.M1", params: { ...P, classId: "c3" }, summary: "m3", requestedBy: "u", requestedRole: "customer" });
  assert.equal(fourth.status, "DENIED");
  assert.match(fourth.deniedReason, /modify_pending_cap/);
  // other tenants unaffected
  const other = await s.create({ tenantId: "8", capabilityId: "pbx.M1", params: { ...P, tenantId: "8" }, summary: "m", requestedBy: "u", requestedRole: "customer" });
  assert.equal(other.status, "PENDING_APPROVAL");
});

test("ROUTING: pbx.M1 goes to the modify backend, pbx.P1 to the legacy backend", async () => {
  const s = svc();
  const m = await s.create({ tenantId: "21", capabilityId: "pbx.M1", params: P, summary: "m", requestedBy: "u", requestedRole: "customer" });
...
test("SINGLE-USE: approving a bound action consumes it; second execute attempt is blocked", async () => {
  const s = svc();
  const a = await s.create({ tenantId: "21", capabilityId: "pbx.M1", params: P, summary: "m", requestedBy: "u", requestedRole: "customer" });
...
test("EMAIL REDEMPTION: bound action requires a bound token with the matching hash", async () => {
  ...
  const wrongHash = computeParamsHash("pbx.M1", "21", { ...P, classId: "other" });
