...
test("campaigns: PATCH and DELETE use requireCrmAccess and campaign scope", () => {
  ...
  assert.doesNotMatch(patch, /requireCrmManager\(req, reply\)/);
  assert.match(del, /requireCrmAccess\(req, reply\)/);
  assert.match(del, /assertCrmCampaignAllowed/);
  assert.match(del, /status: "ARCHIVED"/);
  assert.doesNotMatch(del, /crmCampaign\.delete/);
});

test("funders: PATCH and DELETE use requireCrmAccess (soft-delete)", () => {
...
test("contacts: PATCH and DELETE use requireCrmAccess with contact scope", () => {
  ...
  assert.match(del, /requireCrmAccess\(req, reply\)/);
  assert.match(del, /assertCrmContactAllowed/);
  assert.match(del, /active: false, archivedAt: new Date\(\)/);
  assert.doesNotMatch(del, /contact\.delete/);
});

test("contacts: restore uses requireCrmAccess and contact scope", () => {
  const source = read("apps/api/src/crm/contactRoutes.ts");
  const restore = routeBlock(source, 'app.post("/crm/contacts/:id/restore"', "app.patch(\"/crm/contacts/:id/phones\"");
  assert.match(restore, /requireCrmAccess\(req, reply\)/);
  assert.match(restore, /assertCrmContactAllowed/);
});
