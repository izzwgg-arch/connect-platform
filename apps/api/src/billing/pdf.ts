...
function registerInvoiceFonts(doc: PDFKit.PDFDocument): { regular: string; medium: string; semibold: string; bold: string } {
  try {
    if (fs.existsSync(BUNDLED_INTER_PATH)) {
      doc.registerFont(FONT_REGULAR, BUNDLED_INTER_PATH);
      doc.registerFont(FONT_MEDIUM, BUNDLED_INTER_PATH);
      doc.registerFont(FONT_SEMIBOLD, BUNDLED_INTER_PATH);
      doc.registerFont(FONT_BOLD, BUNDLED_INTER_PATH);
      return { regular: FONT_REGULAR, medium: FONT_MEDIUM, semibold: FONT_SEMIBOLD, bold: FONT_BOLD };
    }
  } catch {
    // Built-in PDF fonts keep invoice generation reliable if the bundled font cannot load.
  }
  return { regular: "Helvetica", medium: "Helvetica-Bold", semibold: "Helvetica-Bold", bold: "Helvetica-Bold" };
}
...
function label(doc: PDFKit.PDFDocument, text: string, x: number, y: number, w: number, color = "#2563eb") {
  doc.fillColor(color).font(FONT_SEMIBOLD).fontSize(7.4).text(text.toUpperCase(), x, y, {
    width: w,
...
function textHeight(doc: PDFKit.PDFDocument, text: string, width: number, size: number, font = FONT_REGULAR, lineGap = 1) {
...
function drawStatusPill(doc: PDFKit.PDFDocument, statusText: string, x: number, y: number) {
  ...
  const width = Math.max(52, doc.font(FONT_SEMIBOLD).fontSize(6.8).widthOfString(statusText) + 18);
  doc.roundedRect(x - width, y, width, 17, 8.5).fillAndStroke(bg, border);
  doc.fillColor(fg).font(FONT_SEMIBOLD).fontSize(6.8).text(statusText, x - width, y + 5.1, { width, align: "center" });
}
...
function drawIconBadge(doc: PDFKit.PDFDocument, kind: IconKind, x: number, y: number, size = 20) {
  ...
  if (kind === "chat") {
    ...
    doc.moveTo(x + 4, y + 11)
      .bezierCurveTo(x + 4.5, y + 5.5, x + size - 4.5, y + 5.5, x + size - 4, y + 11)
      .stroke();
    doc.roundedRect(x + 3.5, y + 10, 3.8, 6.2, 1.5).stroke();
    doc.roundedRect(x + size - 7.3, y + 10, 3.8, 6.2, 1.5).stroke();
    doc.moveTo(x + size - 5.4, y + 16).lineTo(x + size - 5.4, y + 17.3).lineTo(cx + 3.2, y + 17.3).stroke();
    doc.circle(cx + 2.2, y + 17.3, 0.9).fill();
  } else if (kind === "portal") {
...
export async function renderBillingInvoicePdf(invoice: any): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor(ink).font(FONT_SEMIBOLD).fontSize(9.8).text(CONNECT_LEGAL_NAME, billFromX, infoY + 20, { width: 160 });
    let fromY = doc.y + 8;
    for (const value of [CONNECT_SUPPORT_EMAIL, CONNECT_PHONE, CONNECT_WEBSITE]) {
      doc.fillColor(text).font(FONT_REGULAR).fontSize(8.4).text(value, billFromX, fromY, { width: 160, lineGap: 1.1 });
      fromY = doc.y + 5.5;
    }
    ...
    meta.forEach(([k, v, icon], idx) => {
      ...
      doc.fillColor(k === "Due date" && !isPaid ? "#b42318" : ink).font(FONT_SEMIBOLD).fontSize(8.5).text(v, x + 23, y + 15, { width: metaW - 34 });
    });
    y += 47;
    ...
    if (lineItems.length === 0) {
      doc.moveTo(tableX, y).lineTo(tableRight, y).strokeColor(lightLine).lineWidth(0.7).stroke();
      doc.fillColor(muted).font(FONT_REGULAR).fontSize(8.7).text("No line items are attached to this invoice.", tableX, y + 12, { width: tableW, align: "center" });
      y += 36;
    }
    doc.moveTo(tableX, y).lineTo(tableRight, y).strokeColor(lightLine).lineWidth(0.7).stroke();
    y += 16;
    ...
    doc.fillColor(blue).font(FONT_SEMIBOLD).fontSize(9.3).text("Notes", ml + 43, splitY + 14, { width: notesW - 58 });
    ...
    doc.fillColor(muted).font(FONT_REGULAR).fontSize(8.2).text(noteCopy, ml + 16, splitY + 40, { width: notesW - 32, lineGap: 1.4 });
    ...
    buildTotalsRows(invoice, lineItems).forEach((row) => {
      ...
      doc.fillColor(due ? blue : row.tone === "paid" ? "#047857" : ink).font(bold ? FONT_SEMIBOLD : FONT_REGULAR).fontSize(due ? 11.8 : 8.5);
      doc.text(row.label, totalsX + 16, ty, { width: 145 });
      doc.font(FONT_SEMIBOLD).text(money(row.valueCents), totalsX + 186, ty, { width: 76, align: "right" });
      ty += due ? 20 : 16;
    });
    y = splitY + notesH + 22;
    ...
    noticeCols.forEach(([icon, title, body], idx) => {
      ...
      doc.fillColor(ink).font(FONT_SEMIBOLD).fontSize(8.2).text(title, x + 29, y + 14, { width: noticeW - 35 });
      doc.fillColor(text).font(FONT_REGULAR).fontSize(7.3).text(body, x + 29, y + 31, { width: noticeW - 39, lineGap: 0.7 });
    });
    y += regH + 10;
