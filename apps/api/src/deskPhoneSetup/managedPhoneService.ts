...
/**
 * ⛔ The office wizard's own list of Loopcom's provisioning hosts, kept in miniature
 * here rather than imported from `deskPhoneRoutes.ts` — this device-cloud module must
 * never depend upward on the wizard's own route file. Keep in sync with the literal
 * array `registerDeskPhoneSetupRoutes()` is called with in `server.ts` if it changes.
 */
function defaultOurProvisioningHosts(): string[] {
  return ["m.connectcomunications.com", "connectcomunications.com", "loopcom.net"];
}

/** Dot-boundary match only — `loopcom.net.evil.com` is never ours. Same rule as deskPhoneRoutes.ts's classifyOurs(). */
function isOurProvisioningHost(hostname: string, hosts: string[]): boolean {
...
/**
 * ⛔⛔ THE OFFICE WIZARD'S REDIRECT MUST POINT AT ONE OF OUR OWN TENANT PROVISIONING
 * FOLDERS, NEVER ANYWHERE ELSE. A factory-booting phone downloads its SIP credentials
 * from whatever RPS hands it — the same fence `deskPhoneRoutes.ts`'s `classifyOurs()`
 * uses to decide a provisioning URL is genuinely Loopcom's: https, a Loopcom host, and
 * the exact shape a tenant's `/phoneprov/<16 hex>/` folder takes (never a query string,
 * credentials in the URL, or an arbitrary path).
 */
function assertOfficeWizardRedirectUrl(redirectUrl: string, hosts: string[]): void {
  let url: URL;
  try { url = new URL(redirectUrl); } catch { throw new DeviceError("redirect_url_invalid", 400); }
  if (url.protocol !== "https:" || url.username || url.password || url.search || url.hash)
    throw new DeviceError("redirect_url_invalid", 400);
  if (!isOurProvisioningHost(url.hostname, hosts)) throw new DeviceError("redirect_url_invalid", 400);
  if (!/^\/phoneprov\/[0-9a-f]{16}\/$/.test(url.pathname)) throw new DeviceError("redirect_url_invalid", 400);
}
export class ManagedPhoneService {
  constructor(private provider: DeskPhoneProvider = new YealinkProvider(), private database: any = db,
    ...
    private ourProvisioningHosts: () => string[] = defaultOurProvisioningHosts) {}
  private url(mac: string) { return `${this.base()}${mac}/`; }
  /**
   * ⛔⛔ THE URL RPS IS TOLD TO REDIRECT TO. This service's OWN handset route
   * (`this.url(mac)`) for an ordinary managed phone — or, for a phone claimed
   * through the office wizard (`options.redirectUrl`), the TENANT'S EXISTING
   * `/phoneprov/<16 hex>/` folder instead, so a factory-booting Yealink downloads
   * the exact SIP config the office wizard's own PBX record already produces,
   * never a second config generated by this class's own `configuration()` (still
   * rendered and stored below, harmless: nothing ever fetches it at that URL).
   */
  private urlFor(row: { macAddress: string; options?: unknown }): string {
    const redirect = row.options && typeof row.options === "object" ? (row.options as Record<string, unknown>).redirectUrl : undefined;
    return typeof redirect === "string" && redirect ? redirect : this.url(row.macAddress);
  }
  ...
  /**
   * The one place a brand-new row is written. Shared by `provision()` (the managed-phone
   * panel) and `claimForOfficeWizard()` (the office wizard) so a NEW ManagedDeskPhone row
   * is always built the same way, whichever door created it.
   */
  private async createRow(tx: any, actor: Actor, requestId: string, input: {
    mac: string; model: string; extensionId: string; serialNumber: string;
    nickname?: string | null; displayName?: string | null; options?: Record<string, unknown>; replacesId?: string | null;
  }) {
    const sip = await this.loadSip(actor.tenantId, input.extensionId, input.mac);
    ...
    this.provider.configuration(input.model, { ...sip, displayName: input.displayName || sip.displayName }, secrets, this.urlFor({ macAddress: input.mac, options }), input.mac, options as DeviceConfigOptions);
    const row = await tx.managedDeskPhone.create({ data: {
      macAddress: input.mac, tenantId: actor.tenantId, extensionId: input.extensionId,
      manufacturer: this.provider.manufacturer, model: input.model, endpoint: sip.endpoint,
      serialNumber: input.serialNumber, nickname: input.nickname || null, displayName: input.displayName || null, options,
  ...
  async provision(actor: Actor, input: { mac: string; serialNumber?: string; model: string; extensionId: string; nickname?: string; displayName?: string; options?: DeviceConfigOptions; replacesId?: string }, requestId: string) {
    ...
    const device = await this.lock(mac, async tx => {
      ...
      return this.createRow(tx, actor, requestId, {
        mac, model: input.model, extensionId: input.extensionId, serialNumber,
        nickname: input.nickname, displayName: input.displayName, options: input.options || {}, replacesId: input.replacesId || null,
      });
    });
  ...
  /**
   * ⛔⛔ THE OFFICE WIZARD'S ENTRY POINT INTO RPS (Izzy, 2026-09-17: "integrated with
   * both databases"). A Yealink the wizard already knows — MAC off the scan, serial off
   * the label, an extension the customer chose — is claimed into Loopcom's RPS account
   * pointed at the TENANT'S OWN provisioning folder, so a factory-reset of that exact
   * physical phone anywhere with internet is redirected straight to the same settings
   * the office wizard's PBX record already produces. `ManagedPhoneService` stays the
   * ONE RPS writer; this is a second FRONT DOOR onto it, not a second writer.
   *
   * ⛔ `macAddress` is globally unique on `ManagedDeskPhone`, so a MAC that already has a
   * row — ours or another tenant's, retired or not — is always REUSED in place; a
   * `create()` here would violate the unique constraint the instant a row already exists.
   */
  async claimForOfficeWizard(actor: Actor, input: {
  ...
  }, requestId: string): Promise<{ id: string; rpsState: string; lastError: string | null; conflict: boolean }> {
    const mac = strictMac(input.mac); this.provider.validateModel(input.model);
    const serialNumber = String(input.serialNumber || "").trim();
    if (!serialNumber) throw new DeviceError("serial_number_required", 400);
    assertOfficeWizardRedirectUrl(input.redirectUrl, this.ourProvisioningHosts());
    const options = { source: "office_wizard", redirectUrl: input.redirectUrl };

    const deviceId = await this.lock(mac, async tx => {
      const prior = await tx.managedDeskPhone.findUnique({ where: { macAddress: mac } });

      // A genuine first claim: nobody has ever recorded this MAC.
      if (!prior) {
        const row = await this.createRow(tx, actor, requestId, {
          mac, model: input.model, extensionId: input.extensionId, serialNumber,
          displayName: input.displayName, options,
        });
        return row.id;
      }

      const sameTenant = prior.tenantId === actor.tenantId;
      if (!sameTenant && !prior.retiredAt) {
        // ⛔⛔ ANOTHER LOOPCOM TENANT ACTIVELY HOLDS THIS MAC. Mirrors `decideRehome`'s
        // presence-pair rule (packages/shared/.../provisioningRecord.ts, "the wizard owns
        // a phone on its own network"): a move is allowed ONLY when this office machine has
        // actually SEEN the handset on the CLAIMING customer's own LAN (discoveredIp) from
        // THEIR public address (requesterIp) — without both, nothing tells a genuine rehome
        // apart from a forged report naming another company's hardware.
        if (!input.presence?.discoveredIp || !input.presence?.requesterIp) throw new DeviceError("device_ownership_conflict");
        if (prior.rpsState === "assigned") {
          try {
            await this.provider.rps.release(prior.macAddress, prior.rpsServerId || this.serverId(), this.urlFor(prior));
          } catch (e) {
            // ⛔ Swallowed on purpose: the maker's account no longer agreeing the URL is
            // ours to release is exactly the state a rehome exists to fix. The row is
            // reclaimed below regardless, and `reconcile()` re-asserts ownership with RPS.
            if (!(e instanceof DeviceError && e.code === "rps_assignment_conflict_requires_release")) throw e;
          }
        }
        await this.event(tx, actor, prior, "REHOMED_BY_OFFICE_WIZARD", requestId, {
          fromTenantId: prior.tenantId, toTenantId: actor.tenantId,
        });
      }

      // Already exactly this, for us: reuse without a write or a fresh RPS round trip
      // (reconcile() below still runs — cheap and idempotent — to confirm RPS agrees).
      if (sameTenant && !prior.retiredAt
        && prior.extensionId === input.extensionId && prior.model === input.model
        && prior.serialNumber === serialNumber
        && (prior.options as Record<string, unknown> | null)?.redirectUrl === input.redirectUrl) {
        return prior.id;
      }

      // The one row this MAC is allowed to have — ours and stale, or just released
      // above from another tenant — is reused in place, never re-created.
      const sip = await this.loadSip(actor.tenantId, input.extensionId, mac);
      const secrets: Secrets = sameTenant
        ? decryptJson<Secrets>(prior.secretsEncrypted)
        // ⛔ A different tenant NEVER reuses the previous owner's provisioning secret,
        // even though it is inert against the PBX phoneprov folder this points at.
        : { provisioningPassword: randomBytes(24).toString("base64url"), adminPassword: randomBytes(24).toString("base64url") };
      this.provider.configuration(input.model, { ...sip, displayName: input.displayName || sip.displayName }, secrets, this.urlFor({ macAddress: mac, options }), mac, options as DeviceConfigOptions);
      const row = await tx.managedDeskPhone.update({ where: { id: prior.id }, data: {
        tenantId: actor.tenantId, extensionId: input.extensionId, model: input.model, endpoint: sip.endpoint,
        serialNumber, displayName: input.displayName ?? prior.displayName, options,
        secretsEncrypted: encryptJson(secrets), configVersion: { increment: 1 },
        retiredAt: null, replacesId: null, rpsState: sameTenant ? prior.rpsState : "pending_credentials",
        updatedBy: actor.sub,
      } });
      await this.event(tx, actor, row, "CONFIGURATION_UPDATED", requestId, { extensionId: row.extensionId, source: "office_wizard" });
      return row.id;
    });

    await this.reconcile(actor, deviceId, requestId);
    const after = await this.own(this.database, actor, deviceId);
    return { id: after.id, rpsState: after.rpsState, lastError: after.lastError ?? null, conflict: after.rpsState === "conflict" };
  }
  async reconcile(actor: Actor, id: string, requestId: string) {
    ...
    return this.lock(first.macAddress, async tx => {
      ...
      try {
        ...
        const out = await this.provider.rps.assign({ mac: row.macAddress, serialNumber: row.serialNumber || "", serverId: this.serverId(), uniqueServerUrl: this.urlFor(row), authName: row.macAddress, password: secret.provisioningPassword });
  ...
  async update(actor: Actor, id: string, input: { extensionId?: string; nickname?: string; displayName?: string; options?: DeviceConfigOptions }, requestId: string) {
    ...
    await this.lock(first.macAddress, async tx => {
      ...
      this.provider.configuration(row.model, { ...sip, displayName: input.displayName ?? row.displayName ?? sip.displayName }, decryptJson(row.secretsEncrypted), this.urlFor({ macAddress: row.macAddress, options: input.options || row.options }), row.macAddress, input.options || row.options);
  ...
  async release(actor: Actor, id: string, requestId: string) {
    ...
    await this.lock(first.macAddress, async tx => {
      ...
      await this.provider.rps.release(row.macAddress, row.rpsServerId || this.serverId(), this.urlFor(row));
  ...
  async configuration(macInput: string, filename: string, authorization: string | undefined, requestId: string, sourceIp: string, userAgent: string) {
    ...
    return this.lock(mac, async tx => {
      ...
      const config = this.provider.configuration(row.model, { ...sip, displayName: row.displayName || sip.displayName }, decryptJson(row.secretsEncrypted), this.urlFor(row), mac, row.options);
      const now = new Date();
  ...
  async completeReplacement(actor: Actor, id: string, requestId: string, attestedWorking = false) {
    ...
    await this.database.$transaction(async (tx: any) => {
      ...
      if (old.rpsState !== "pending_credentials" && old.rpsState !== "released")
        await this.provider.rps.release(old.macAddress, old.rpsServerId || this.serverId(), this.urlFor(old));
