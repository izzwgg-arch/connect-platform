...
// ─── Ring-leg-abandon → diverted_to_voicemail (killed-app stop) ────────────
//
// Reproduces the swiped-away regression root cause: the PBX rolls to voicemail
// WITHOUT any "voicemail" channel name (T<id>_cos-all ring group), so the old
// `looksDivertedToVoicemail` never fired and the killed app rang until hangup.
// The notifier must watch the LIVE channel set: once the rung extension's legs
// are all gone while the trunk stays up, it fires a single diverted_to_voicemail
// stop after the debounce window.

const ABANDON_WAIT_MS = 1700; // RING_LEG_ABANDON_MS (1500) + scheduling slack

async function wait(ms: number): Promise<void> {
  await new Promise<void>((resolve) => setTimeout(resolve, ms));
}

test("ring leg abandoned (no voicemail channel) fires one diverted_to_voicemail after debounce", async () => {
  const { calls, restore } = installFetchSpy();
  try {
    const notifier = new MobilePushNotifier();
    // 1) Ring: trunk + extension leg present → one ring push.
    notifier.notify(
      makeCall({
        linkedId: "abandon.1",
        direction: "inbound",
        ...
        to: "8455577768",
        extensions: ["T21_101"],
        channels: ["PJSIP/344022_Comfortcont-0000f2a5", "PJSIP/T21_101_1-0000f2a6"],
    ...
    assert.equal(calls[0].body.toExtension, "101");

    // 2) Voicemail answers the trunk: extension leg gone, trunk stays up.
    //    `extensions` stays cumulative (sticky) — detection must use channels.
    notifier.notify(
      makeCall({
        linkedId: "abandon.1",
        direction: "inbound",
        ...
        to: "8455577768",
        extensions: ["T21_101"],
        channels: ["PJSIP/344022_Comfortcont-0000f2a5"],
    ...
    await flush();
    assert.equal(calls.length, 2, "exactly one diverted stop after debounce");
    assert.equal(calls[1].body.state, "diverted_to_voicemail");
    assert.equal(calls[1].body.linkedId, "abandon.1");
  } finally {
    restore();
  }
});
...
test("fork re-INVITE within debounce window cancels the stop (no false diverted)", async () => {
  ...
  try {
    const notifier = new MobilePushNotifier();
    const ring = (channels: string[], state: CallState = "ringing") =>
      notifier.notify(
        makeCall({
          linkedId: "fork.1",
          direction: "inbound",
    ...
    ring(["PJSIP/344022_Comfortcont-0000f2a5", "PJSIP/T21_101_1-0000f2a6"]);
    await flush();
    assert.equal(calls.length, 1, "initial ring push");

    // Fork hand-off: ext leg momentarily gone …
    ring(["PJSIP/344022_Comfortcont-0000f2a5"]);
    await flush();
    // … then re-INVITE brings a fresh ext leg back well within the window.
    await wait(600);
    ring(["PJSIP/344022_Comfortcont-0000f2a5", "PJSIP/T21_101_1-0000f2b0"]);
    await flush();

    await wait(ABANDON_WAIT_MS);
    await flush();
    assert.equal(calls.length, 1, "no diverted stop — the ring leg came back");
  } finally {
    restore();
  }
});
