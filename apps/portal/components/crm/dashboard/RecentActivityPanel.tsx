...
export function RecentActivityPanel({ items }: { items: ActivityItem[] }) {
  return (
    <CRMCard className={cn("p-5", crm.opCard, "border-crm-border/80")}>
      ...
      {items.length === 0 ? (
        <div className="rounded-crm border border-dashed border-crm-border/70 bg-crm-surface-2/25 px-3 py-4">
        ...
        <ul className="m-0 flex list-none flex-col gap-1.5 p-0">
          {items.map((it) => (
            <li key={it.id} className="group flex items-center gap-2 rounded-crm border border-crm-border/60 bg-crm-surface-2/40 px-2.5 py-1.5 text-sm text-crm-text transition-all duration-200 hover:-translate-y-px hover:border-crm-border hover:bg-crm-surface-2/70">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-crm border border-crm-border/55 bg-crm-surface/60 text-crm-accent">
