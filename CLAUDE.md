...
- 2026-09-17 · YIDDISH LEARNING ENGINE + YIDDISH24 — AUDIO ON by owner permission, off-box PC runner, 24/7 download un-stalled (api `51072578`): Izzy authorized Yiddish24 audio (site owner) + customer voicemail audio (carrier) — 2 `YcRightsRecord` grants with his quoted words; off-box runner `scripts/yiddish-runner/` (runs on the PC) claims `fetch_audio,segment,features` as `izzy-pc`, keeps audio on the PC, writes back over an SSH tunnel; server skips those stages (`YIDDISH_WORKER_EXCLUDE_STAGES`, env-verified). Live: 34 Yiddish24 episodes + 19,163 acoustic segments. ⛔⛔ NOT learning language yet — no `transcribe`/`align`/`cluster` handler exists, no STT wired, so `YcTranscript`/`YcLexeme`/`YcPronunciationRule`/`YcFinding` all 0; acoustic-only. Real remaining build = Yiddish STT (YiddishLabs=no-train / Whisper=weak) + alignment + clustering + a paid budget; no API spend possible or made. ⛔ download stalled 13:30→15:28Z (runner claims fetch across ALL sources; paused voicemail jobs starved Yiddish24) — fixed by parking voicemail jobs (`nextRunAt`→2027, reversible). Customer-voicemail leg authorized+registered (1,888) but PARKED on purpose. ⛔ deploy the origin tip → `2026-09-15-yiddish-learning-engine.md`
