...
- 2026-09-17 · VITALPBX DASHBOARD BANNERS (blue "Unlock premium" ad = the canceled subscription; yellow "global limits" = OUR deliberate RTP widening, never lower it) — panel's `showStartUp()` only draws them when cookies `is_free_version`/`verify_global_limits` are MISSING, so the fix is two 10-yr cookies in Izzy's browser (paste in the handoff); ⛔ m.connectcomunications.com is the PBX directly, no proxy — any server-side fix = forbidden PBX write; ⛔ agent's cookie-write + browser-switch were classifier-DENIED in auto mode; ⏳ Izzy runs the paste, next login proves it → `2026-09-17-vitalpbx-dashboard-banners.md`
