...
- 2026-09-18 · CUSTOMER COST BREAKDOWN — ✅ BUILT + DEPLOYED (portal `6d0fa490`, api `57d9c7b9`, migration applied, container-verified; PROVEN on Gesheft Aug 5–Sep 3: cost $169.78 on $450.41 = 62% margin, CNAM $39.59 = 23% of cost): office-only "What this customer cost us" card on every invoice (minutes in/out, texts, CNAM lookups, numbers/911, one-time, margin) + drill-down by number/day + Catalog "What the carriers charge us" + customer month-by-month; VoIP.ms feed LIVE (getCDR `total` per call, `call_logs` names sub-account + "Doing a CNAM lookup" → CNAM per tenant at $0.008, reconciles exactly; DID/E911 from transactions; 6-second billing) pulling every 6 h + Aug 3–Sep 3 backfill; ⛔ outbound = shared Telocall 0001, NO API → our minutes × $0.006 (Izzy "00.06"); ⛔ never getSubAccounts (SIP passwords); every line carries an honesty tier; ⏳ Izzy hasn't opened it; mockups https://claude.ai/artifact/BMtViAYuCsesLP9gNCYLVx → `2026-09-18-customer-cost-breakdown-mockups.md`

- 2026-09-18 · LOOPCOM COMMUNITY — **BUILT, NOT DEPLOYED, FULL SUITE GREEN** (Izzy: separate server later, Loopcom SSO = no second login): `apps/community-api` (own db, 387 routes, 243 integration tests on the real db), `apps/community-web` (Next 15.5, Connect tokens light+dark), `apps/community-mobile` (Expo, 65 tests, bundles); Tier-1 17 flows×20 = 340/340, chaos 5/5, isolation 5/5, controls 786/786, **Playwright E2E on the production build: desktop 47+1 skip · iPhone/WebKit 48/48 · Android/Pixel 48/48** (the phone runs found 9 defects desktop missed — ⛔ every stacking grid must be `minmax(0, 1fr)` or one wide table pushes the page off a phone screen), 10-min soak 28k req / 0 errors / memory flat; run docs `docs/community/README.md`, `docs/community/COMPLETION_REPORT.md`; ⛔ never deploy to the Connect box, ⛔ never `prisma migrate dev` there, ⛔ community binaries are `binary` in .gitattributes (the LF rule was corrupting PNGs); ⏳ 1-h soak on the target box, real-device mobile, keys (SMTP/Google/Apple/Anthropic), portal-side sidebar link → `2026-09-18-loopcom-community-built.md`
