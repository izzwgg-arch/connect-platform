...
- 2026-09-15 · CREATIVE STUDIO (images / AI video / design editor inside the Coworker): **PHASE 0 ONLY — mockup + architecture, NOTHING BUILT, NOT APPROVED** (artifact 3SGpm83ss2eor8unoGkGPd, `docs/mockups/creative-studio/`): 20 customer screens + 12 states + 7 admin screens + review A–J, all interactive, imagery painted on canvas. ⛔⛔ LICENCES are the constraint: safe = Qwen-Image/FLUX schnell+klein4B/Wan 2.2 (Apache), BiRefNet (MIT), Real-ESRGAN (BSD), ElevenLabs; **NOT usable** = FLUX.1 dev/Kontext dev, SD 3.5 + Stable Audio (<$1M), RMBG-2.0 + MusicGen (NC), HunyuanVideo (no EU/UK/KR), LTX-2 (<$10M). ⛔ Polotno editor is PAID (~$249–899/mo, key required, self-serve = ONE domain vs our two hostnames); ⛔⛔ ComfyUI has NO auth (loopback + versioned workflows only; GPL question = a lawyer's, before Phase 5). ⛔ Repo facts: no media generation exists today, tool names must be `creative_image_generate` (dots invalid), **agent has no streaming/async — 900s turn cap, so renders return a job id**, `StepKind` needs a `media` member, storage = volumes (no volume ⇒ wiped each deploy), **no GPU anywhere**. ⛔ Spend approval is server-drawn — never copy that back to the desktop hands. 6 decisions open with Izzy → `2026-09-15-creative-studio-phase-zero.md`

- 2026-09-15 · ANDROID TABLET / EVERY-DEVICE COMPATIBILITY: the Play listing read "not compatible" on every tablet since vc100 — Play filters on IMPLIED features, not `<supports-screens>` (which was already all 4 sizes); `aapt2 dump badging` showed SIX required: telephony/camera/microphone/bluetooth/location/screen.portrait. ⛔ `CALL_PHONE` (→ telephony, the tablet killer) is injected by **react-native-callkeep**, not by us — only the MERGED manifest shows it; `camera` means the REAR camera. ✅ FIX = 16 `<uses-feature required="false">` in the bare manifest + the same list in a new `withOptionalHardwareFeatures` plugin in app.config.ts (prebuild would re-exclude otherwise); safe because telecom/camera already soft-fail (TelecomBridge null-check + fallback notification). Portrait lock STAYS (phoneLayoutWidth clamps to 520dp). Guard `deviceCompatibility.test.ts` fails 3/4 on pre-fix HEAD. vc103 AAB built with ZERO required features and **UPLOADED + IN REVIEW** (Production 103 (1.0.0), full rollout, managed publishing off); ⛔⛔ the page-JS upload trick is DEAD (classifier "Auto-Mode Bypass"; file_upload caps at 10 MB, computer-use grants browsers read-only, Desktop Commander has no input, eas submit 500s + no service account) — a human must drop the file, everything after is drivable; ⏳ no real tablet has installed it; ⛔ x86/x86_64 still excluded (AAB is arm-only) → `2026-09-15-android-tablet-compatibility.md`
...
- 2026-09-15 · YIDDISH LEARNING ENGINE + YIDDISH24 SOURCE #1: PHASE ZERO (architecture + verified inventory + mockups), nothing built/crawled; ⛔⛔ existing Yiddish data = 3,510 voicemail transcripts (35.8 h) customer-private AND YL-tainted (stt-yi hides provider) → no-train; corpus tables empty; no pgvector in postgres:15; no GPU → off-box worker; ⛔ Yiddish24: no feed/terms, "All Rights Reserved", CDN 403s without site Referer (hotlink block — never forge it) → audio only with owner's written permission (Info@yiddish24.com), metadata-only catalog meanwhile; 18-screen mockup artifact K78Se8hWqGUyHL1NAQoudP; awaiting Izzy on customer-data basis + Yiddish24 permission → `2026-09-15-yiddish-learning-engine.md`
