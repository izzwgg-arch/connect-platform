...
- 2026-09-18 · YIDDISH TRAINING RUNS + BULLETIN GOLD — **run 7 is the FIRST to get past a single optimizer step** (4 dead runs: dataset never attached — `kaggle kernels push` exits 0 while printing "not valid dataset sources"; a STALE `train.py` because the CODE dataset was pushed but never in `dataset_sources`; `dtype=` vs `torch_dtype=`; LoRA+gradient-checkpointing severed the graph) — all four now guarded/fixed + `watch-training.ts` writes the REAL outcome with root-cause lines into `YcPipelineState` (the portal said "running" through three dead runs). ⛔⛔ TWO process bugs cost more: the relaunch watcher's PowerShell filter MATCHED ITSELF so it never relaunched for 3 h, and the orchestrator ran with an EMPTY `DATABASE_URL` (the runner `.env` lives in `~/LoopcomYiddishRunner/`, not the repo) — `loadRunnerEnv()` now, all 4 callers. ✅ BULLETIN GOLD BUILT (Izzy's find): Yiddish24 בולעטין types out what it records — the FIRST labels our model did not write (it heard `דאס רעדן` for `אפטרעטן`, dropped `קיר`); harvest + LCS alignment + a split into `publisher` (no timing, INERT) vs `publisher_aligned` (audio timing, trainable) so a 97%-accurate article never reaches the trainer whole; 29 tests. ⛔ Business/Interviews have NO typed text (Izzy asked for them). ⛔ The first parser keyed on `content_block` and silently lost 70% of the archive as "empty pages". ⏳ nothing in the DB yet, no trained model yet → `2026-09-15-yiddish-learning-engine.md` + `AGENT_HANDOFF_YIDDISH_BULLETIN_GOLD_2026-09-18.md`
