...
- 2026-09-15 · SUPPORT WATCHER — stopping ONE ticket (44B6TB): ⛔⛔ requeue returns BEFORE the cap check ("the cap will stop it" is FALSE); ⛔⛔ loadState() runs ONCE, so a hand-edit to .watch-state.json is SILENTLY CLOBBERED — kill the watcher first; ⛔⛔ "my ticket settled" ≠ "watcher idle" (next claim came 1s later) — read .watch-heartbeat.json, my kill hit a live CUSTOMER run (GEAGCD, auto-requeued but it SPENDS its one retry); ✅ 44B6TB stopped_by_owner; ⛔⛔ but 877-220-5058 is an ORPHAN (0 calls, no customer) and the SAME alarm files a NEW ref every 6 h — stopping one ticket is not stopping the noise; release/route it or give the guardrail an ignore list → `2026-08-31-a-customer-s-support-request-now-opens-a-claude.md`
...
- 2026-09-16 · CREATIVE STUDIO BUILT + LIVE (api `8a914d26`, migration `20260916140000`, 19 additive tables): images and AI video really generate, proven on prod — ⛔⛔ a **15s shot from a 12s engine** (Sora takes seconds ∈ {4,8,12}, so it plans 12+4, continues from the last frame, joins, trims → exactly 15.000000s); trademark ask → 422, other company → **404 not 403**, tampered URL → 401; learning loop proven (3 rejections → active → next prompt carried it). ⛔⛔ Worked because the EXISTING OpenAI key has gpt-image-2.5 **and sora-2**, MinIO was already running+unused, FFmpeg already in the images; ⛔ containers hold a PLACEHOLDER key — real ones are in AgentSecret. ⛔ `/internal/agent/*` needs `jwtPublicRouteBypass.ts` or it 401s. ⏳ portal NOT deployed (build lock), agent NOT rebuilt, no key granted (granting = launch), MinIO on root creds. 40 tests → `2026-09-16-creative-studio-built.md`
...
- 2026-09-16 · YIDDISH LEARNING ENGINE + YIDDISH24 — **BUILT END TO END** (`88682602`): 21 `Yc*` tables + migration `20260916180000`, `apps/api/src/yiddishCorpus/` (34 routes, leased 24/7 worker, Yiddish24 adapter, ffmpeg audio pipeline, benchmark), 10 portal pages `/admin/yiddish-learning` (10 keys, platform-staff Locked), `yiddish-corpus` volume; runs METADATA-ONLY today (≥2s/request, audio stages SKIPPED with a reason); ⛔⛔ three walls are code+guard-tested: customer data counted-never-read, YL serving-only (unprovable `stt-yi` = YL), external audio needs OWNER_AUTHORIZED **and** a GRANTED rights record (the Referer literal lives once, inside the gate); 127/127 tests, migration test-applied to a throwaway DB, parser proven on the LIVE site (10 episodes, 136 series); ⚠️ 36 api tests fail locally from a STALE `packages/integrations/dist` (2026-05-24 artifact), not ours; ⏳ Izzy decides customer-data basis + Yiddish24 permission → `2026-09-15-yiddish-learning-engine.md`
...
- 2026-09-15 · DISPLAY DECKS SPLIT: old tenant renamed NEXUS REALTY (Michael TENANT_ADMIN, Amex default, autopay OFF — his $65 Sola schedule is LIVE, next Sep 26); new billing-only tenant DISPLAYDX (Ellie, Visa default, autopay ON, $30/mo); Ellie's 3 missed cycles CC-202609-00007/8/9 emailed-then-CHARGED on his GO, all PAID $90, receipts SENT; ⛔⛔ never a 2nd TenantPbxLink on one PBX tenant (app stops ringing); ✅ REAL PBX SPLIT PREPPED, NOT LIVE: PBX tenant 142 `displaydx` linked to DisplayDX, ext 101 (PBX 668) + ring groups 800–807 field-for-field = T6's, Connect IVR drafts Displaydex/Quick sat (0 publishes/0 mappings), all 20 applies re-baked clean, Ellie's numbers still on T6; ✅ FULL BACKFILL COPIED before the switch (1,289 contacts, 8 SMS threads + files, 13 voicemails + spool; originals untouched, 0 emails/escalations/pushes fired, T142 sync upserted onto copies; ConnectCdr can't be copied → moved at switch); ⏳ switch night = handoff §6a (re-sync + move + `--remove-originals`), scripts dry-run clean on loopcom /root; 845-414-3736 + ext 104 stay NEXUS → `2026-09-15-displaydx-nexus-realty-split.md`
