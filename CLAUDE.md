...
- 2026-09-17 · GESHEFT PAY LINE — IZZY'S FULL SPEC BUILT + DEPLOYED (api `50f57f76` → round 2 `528bf977`, container-verified, live proof ×3: own-account-unservable callers are ASKED for the account, once): matched caller-ID NEVER keys a PIN (enrolled → silent; own account unservable — no POS PIN or a PIN Loopcom doesn't hold — → **asked for the phone number of the account to pay / hear the balance on** (`34_enter_account_phone`, once per call; Izzy's ask after his no-PIN cell was handed off) and continues like an unknown caller; desk enrolls PINs on Orders → Phone PIN; `SUPERMARKET_PAY_MATCHED_PIN_POLICY=ask_once` restores ask-once); unknown caller keys the account number → silent probe → `23_pin_or_star`; star → 6-digit code by PHONE CALL (dialplan `Originate` through Gesheft's own trunk, CID 845-244-9666, Stephen's voice) or TEXT (from +18452449666), only to a number on the account, picked by last four; correct code = served like a matched caller; ⛔⛔ the register refuses CHARGES on no-PIN accounts with or without a header (re-proven) so no-PIN accounts stay unservable and Izzy's 3064 line needs its POS PIN enrolled once; ⛔ Stephen = Polly NEURAL not ElevenLabs — 12 new prompts cut + installed (en-male 52→64), dialplan spliced (`.bak.paycode.20260917T175136Z`); tests **224/224** (payLineCode 13 NEW incl. a 500-account stress); ✅ live door proof ×5 on the deployed container (matched 3762 → person + `pin_not_enrolled`, NO 02_pin; star+1 → `dial=8457823064` + say-list; star+2 → a REAL text left 845-244-9666; 0 codes persisted, vault 0); ⏳ a HUMAN code call/text + a real charge; the pay line charges STRAIGHT THROUGH THE POS (no Sola) — Sola remains only on the Orders desk's new-card capture → `2026-09-17-pay-line-code-by-call-or-text.md` (prior, still true for the register facts: `2026-09-17-pay-line-caller-id-rule.md`)
