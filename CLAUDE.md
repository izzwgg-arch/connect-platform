...
- 2026-09-15 · SignalWire DAY TEST: 6775 ROLLED BACK (§5, verified), test v2 now on A plus 845-782-3064 (§7; TRUNK_ID=132 guard IS the anti-loop; CID survives only via the 0001 hop); ⛔⛔ answered calls DROP (cause 58) — PCAP-PROVEN SRTP MISMATCH (SignalWire offers SAVP+crypto, trunk `media_encryption=no`, Asterisk's invalid hybrid SDP dies at the answer-time re-INVITE), NOT direct media; ✅ FIX APPLIED 11:05 ET: `media_encryption=sdes` via regen-safe `pjsip__60_custom.conf` (⛔ dashboard CANNOT disable SW encryption; "Optional" still offers SAVP; `(+)` append hits the FIRST same-named section = the endpoint, verified) ⏳ answered-call proof pending — MIGRATION RULE: every SW trunk needs sdes → `2026-09-15-signalwire-day-test-aplus-6775.md`
