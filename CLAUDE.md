...
- 2026-09-17 · GESHEFT PAY LINE — THE CALLER-ID RULE BUILT + DEPLOYED (api+portal e6875027): matched caller-ID keys NO PIN (enrolled → silent; none → silent register probe); ⛔⛔ register `"Customer PIN required."` = the POS has NO PIN on that account, NOBODY can be served → person at once + `status no_pin` (Izzy's 562-209-6644/1001021 and 845-238-0884/4322 are BOTH no-PIN, no-card accounts — every PIN he keyed was futile, not a bug); `"Invalid customer PIN."` = PIN exists → asked ONCE, enrolled per account, silent after; foreign number → PIN every time, never enrolled; mirror fallback for second phones/outages; desk "Phone PIN" control (verify-before-store); 105/105 + 11/11 + payLineStress; ✅ live door proof ×4 scenarios; ⏳ human call with a real POS PIN + card; store must set POS PINs (on-account sample 12/15 have one) → `2026-09-17-pay-line-caller-id-rule.md`
