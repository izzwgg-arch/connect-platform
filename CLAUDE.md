...
- 2026-09-16 · SIGN-UP WIZARD LOOPCOM REBRAND — MOCKUPS ONLY (artifact KHYzqfMVjWgnwE5Gbh5MmX): live wizard still shows a purple square + "Connect" on its own palette; Option A (recommended) = sign-in-page skin, Option B = desktop brand rail; ⛔ one wordmark file both themes; ⏳ Izzy picks, nothing built → `2026-09-16-signup-wizard-loopcom-rebrand-mockups.md`
...
- 2026-09-17 · Desk Phone Wizard device identification + maker clouds; ⛔ NEVER asks a phone password — serial via typing/PHOTO/TEXT through ONE gate `recordLabel`; ✅ the FIRST real photo (Izzy's T42S, upside-down) beat all four OCR passes → **BARCODES ARE READ FIRST now** (zxing, checksum-backed, `4011fa5f`), OCR is the fallback; provision reuses the vouched serial ON FILE; Yealink managed models 7→37 (his T42S had no row); ⛔ his T42S MAC is RPS-claimed by ANOTHER org (800004; LAN PnP still wins at factory boot); ⛔ GDMS add is a BATCH (`3040f2bc`); ⛔⛔ wizard OWNS a phone on its own network (`7e427c28`); ✅ ROUND 18 — the CUSTOMER SCANS THEIR OWN PHONES: public-by-token `/phone-setup/<token>`, camera frames decoded SERVER-side through the same one gate, matched BY MAC and refused by maker name, four approved vendors only, one live revocable link per run; R20 = ON-DEVICE zxing scanner (server stays the only judge, wasm from OUR origin); ⛔⛔ R21 "nothing scans" ROOT CAUSE = portal CSP lacked `'wasm-unsafe-eval'` so `WebAssembly.instantiate` THREW (a wasm 200 ≠ it runs) — FIXED LIVE in `/etc/nginx/connectcomms/security-headers.conf` line 17 (server-only state, no deploy reverts/restores it), post-fix instantiate proven in real Chrome; ⏳ real-sticker human decode still unproven → `2026-09-14-desk-phone-wizard-automatic-device-identification.md`
