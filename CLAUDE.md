...
- 2026-09-16 · UNIFIED MESSAGING (SMS/MMS/RCS/WhatsApp/Messenger over Telnyx/SignalWire/VoIP.ms) — **MOCKUPS ONLY, awaiting Izzy; NOTHING BUILT** (his explicit gate: mockups → approval → build). Full set in `docs/mockups/unified-messaging/index.html` + artifact 4JJZYVHE9KKkQ1XpQm3eQe: 14 web screens + iPhone/Android frames, capability-registry composer, honest state machine (⛔ VoIP.ms has NO delivery receipts — "Sent" max), fallback chips, per-number routing, migration flow w/ rollback. Codebase truth inside: extend `ConnectChatThread`/`TenantSmsNumber.provider`/the ONE send door + ONE ingest registry — ⛔ worker dispatch is a hardcoded if/else (connectChatSmsJob.ts:116), the poll path duplicates the ingest tail, Telnyx has ZERO chat wiring, WhatsApp is simulated + schema-drifted, no realtime anywhere → `2026-09-16-unified-messaging-mockups.md`
