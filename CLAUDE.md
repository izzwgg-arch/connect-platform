...
- 2026-09-16 · YIDDISH LEARNING ENGINE + YIDDISH24 — RUNNING 24/7 until Izzy says "stop listening Yiddish24" (`88682602` → `fb563e27`): WHOLE CATALOG WALKED (136 series, 32,210 items, ~11,084 h), hourly re-check for new episodes; METADATA ONLY — ⛔⛔ 0 audio bytes ever, audio needs OWNER_AUTHORIZED **and** a GRANTED grant, the CDN hotlink block is never worked around. ⛔ SEVEN defects only production showed — incl. `observe`/`aggregate` had NO HANDLER (a `no handler` SKIP reads as lawful) and the crawler PAUSED ITSELF the day it finished ("no new episodes" tripped the 2-empty-runs broken-site alarm; a run now counts empty only if it recognised NOTHING). ⏳ R/CH accent: A/B ear test sent (gpt-4o-mini-tts ± uvular-R/guttural-CH instructions) — steering, not training; verdict pending → `2026-09-15-yiddish-learning-engine.md`
