...
- 2026-09-16 · YIDDISH LEARNING ENGINE + YIDDISH24 — BUILT and RUNNING 24/7, then HARDENED BY READING THE LIVE SYSTEM (`88682602` → `2627b1be`): 21 `Yc*` tables, `apps/api/src/yiddishCorpus/`, 10 pages `/admin/yiddish-learning`; METADATA-ONLY (audio SKIPPED with a reason). ⛔⛔ Three walls code+guard-tested: customer data counted-never-read, YL serving-only, external audio needs OWNER_AUTHORIZED **and** a GRANTED grant (Referer literal once, inside the gate). ⛔ SIX defects a green suite could not see — category was the CSS colour `darkred`; discovery held the ONLY worker lane; 5 jobs/min = weeks; `observe`+`aggregate` HAD NO HANDLER (the `no handler` SKIP read as lawful); a grant would NOT have reached the backlog; a walk in progress never learned the category map. ⛔ Pacing env is LOCAL-only — the ≥2s gap / 30-per-min ceiling / 429 stop are NOT env-tunable. 157/157 tests, 21/21 prod stress, every route + all 10 pages 200 → `2026-09-15-yiddish-learning-engine.md`
