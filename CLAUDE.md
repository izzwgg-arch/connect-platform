...
- 2026-09-18 · LOOPCOM WORKS (TrimPro → Loopcom product, `Loopcom works/` in THIS repo, branch `works/loopcom-works`, worktree `C:\dev\projects\c2-works`) — INVENTORY DONE (8 Sonnet audits under `docs/ai-context/loopcom-works-audit/`: 787 TrimPro strings, 3,990 hard-coded colours + dead dark mode, **13 CRITICAL security findings** incl. unauth’d set-password + unsigned Sola payment webhook + 6 cross-tenant IDOR writes, real mobile app = `apps/mobile/` Expo 54), ARCHITECTURE DECIDED (immutable-id identity map, 60-s single-use SSO code exchanged server-to-server, seamless nav not iframe, `/api/v1/works/*` HMAC service API, NEW outbound signed-webhook dispatcher for screen pop, agent gets a third `resolveIdentity()` branch + `worksTools.ts` — standalone orgs use `works:<orgId>` since `AgentConversation.tenantId` has no FK, Docker Compose deploy per `infra/community/`), mockups https://claude.ai/artifact/LZvf79qD8f5aF3nfQ55nDy (30 boards, repo copy `docs/mockups/loopcom-works/mockups-v1.html`); **✅ PHASE 2 (looks-only reskin + rebrand) BUILT the same evening on Izzy's "build it"** — palette-by-CSS-vars so 153 pages changed with zero JSX edits, portal tokens/shell/login/dark mode, Signal Core logo+favicon+manifest, 59-file string sweep, mobile app rebranded; PROVEN: prod `next build` ✓, tsc = baseline 207, node:test 154/156 (= baseline), click-through E2E on the prod build 40/41 (form login, 25 pages, client create/edit, PDFs, theme/collapse persist, search, logout; 20× run in TESTS_RUN.md); dev server `npm run dev:3001`, prod build test URL `http://works.localtest.me:3002` (localhost redirects in prod — TrimPro's canonical rule); repo hygiene done (nested 3.8 GB `.git` moved to `C:\dev\projects\_backups\`, junk gitignored, legacy DB password scrubbed); ⛔ `prisma/migrations/*/migration.sql` is gitignored in the copy; ⛔ TrimPro prod `154.12.235.86` untouched/out of scope; ⏳ security fixes + Loopcom integration wait for his go; six decisions are Izzy’s (§5) → `2026-09-18-loopcom-works-audit-architecture-mockups.md`

- 2026-09-18 · CUSTOMER COST BREAKDOWN — ✅ BUILT + DEPLOYED (api+portal `6d0fa490`, migration applied, container-verified): office-only "What this customer cost us" card on every invoice (minutes in/out, texts, CNAM lookups, numbers/911, one-time, margin) + drill-down by number/day + Catalog "What the carriers charge us" + customer month-by-month; VoIP.ms feed LIVE (getCDR `total` per call, `call_logs` names sub-account + "Doing a CNAM lookup" → CNAM per tenant at $0.008, reconciles exactly; DID/E911 from transactions; 6-second billing) pulling every 6 h + Aug 3–Sep 3 backfill; ⛔ outbound = shared Telocall 0001, NO API → our minutes × $0.006 (Izzy "00.06"); ⛔ never getSubAccounts (SIP passwords); every line carries an honesty tier; ⏳ Izzy hasn't opened it; mockups https://claude.ai/artifact/BMtViAYuCsesLP9gNCYLVx → `2026-09-18-customer-cost-breakdown-mockups.md`

- 2026-09-18 · LOOPCOM COMMUNITY (Jewish-focused B2B professional network, Izzy’s full brief) — DESIGN ONLY: audit of the existing system (`User` can’t be the Loopcom ID → new `CommunityPerson`; Direct’s identity/policy is the template; reuse realtime/S3/mfa/push/EmailJob/agent LLM lane; extend `apps/mobile`, no second native app), target architecture + domain model + RBAC + build order in the handoff; **24-screen mockups on Connect’s tokens, light+dark: https://claude.ai/artifact/NKRSEmuPNncn8fq3h9z7yW** (repo copy `docs/mockups/loopcom-community/mockups-v1.html`); ⏳ nothing built — phase 1 starts on Izzy’s approval → `2026-09-18-loopcom-community-audit-and-mockups.md`
- 2026-09-18 · GESHEFT PAY LINE — "INVALID PIN BEFORE I KEY ANYTHING" = THE POUND KEY LANDS ON THE NEXT PROMPT (3 of the last 4 real calls: the 10-digit gather closes on the 10th digit, the `#` the prompt asked for became the PIN `Read()`'s terminator → empty → scored WRONG; same defect in the card AGI — expiry closes on 4 digits, the `#` hit the CVV → "that does not look right" ×3 → hang-up = "it's looking for a number"; "too long" = the AGI's 15 s timeout, api steps are 0.04–0.62 s): ✅ FIXED IN THREE LAYERS — reducer `noInputAttempts` (empty answer = replay its own prompt, never wrong, 3 in a row → person), dialplan re-asks an early-empty `Read()` once locally (`stray`, <6 s) + posts `${PAY_CARD}` after the AGI, AGI `collect()` counts empty/wrong apart + timeout 15→10 s; tests 248/248 (7 NEW + 3 guards replay-FAIL on HEAD), AGI self-test 9/9; ⛔⛔ FOUND IN PASSING: VitalPBX logs `dtmf` on `full`/`console`/`fail2ban` → EVERY keyed card number + PIN is in `/var/log/asterisk/full` digit by digit (the 09-17 grep looked for the contiguous number) — logger change is outside the pay-line mandate, IZZY'S CALL, not done; "fast-forward/skipped words" = interruptible prompts by design + the open 09-17 Lumen-path loss; ✅ api `816e5b00` deployed + container-verified, PBX block + AGI live (backups kept), PROVEN on a real originated call (`C-00000029`: the exact 06:49 sequence → "stray terminator, prompt again" → PIN served, 0 wrong-PIN); ⏳ Izzy's own call → `2026-09-18-pay-line-stray-pound-no-input.md`
- 2026-09-18 · GESHEFT 845-244-9666 → TELNYX ZERO-DOWNTIME PORT — ✅ SUBMITTED 09-18 18:55Z: Telnyx order `64106b7c` `in-process`, FastPort, **FOC Tue 09-22 07:00 ET**, LOA signed "izzy wein" + VoIP.ms invoice (names the DID) attached, PIN 9593; ⛔ `emergency_address_id` refused 422 (Emergency ToS not accepted) → **911 on 244-9666 dies at FOC until Izzy accepts the ToS + enable_emergency with address `3051869631594628476`**; ⏳ who is 845-662-3885; at FOC watch switcher flip → campaign attach (~2 h) → CNAM → human call+text. ⛔⛔ FOUND+FIXED api `9a27c130`: BOTH Telnyx webhook doors 401'd every real delivery as "unsigned" (fastify-raw-body onRoute never saw synchronously-registered routes) — after the fix, LIVE-PROVEN on Loopcom's own numbers: Telnyx inbound SMS 2 s, inbound MMS image, outbound via the chat door 6 s to a VoIP.ms number WITHOUT a campaign — ⛔ but to a T-MOBILE CELL the same number is BLOCKED (40010 not 10DLC-registered), so the ~2 h post-FOC outbound gap is a hard block to cells; voice note m4a byte-identical to VoIP.ms and `sent` to T-Mobile (⏳ Izzy's ear), DLRs; ⛔⛔ PLATFORM TEXTING (pay links/receipts/SIGN-IN CODES) DEAD since 723-1213 ported 07:04 ET — VoIP.ms `invalid_did`, needs a new BILLING_SMS_FROM_NUMBER (305-0012 or 557-7768) + api+worker redeploy; port_out_pin 9593; VoIP.ms sms_forward→662-3885 stops at FOC; outbound-SMS gap ~2 h (Telnyx refuses pre-port attach) on ~1 text/month → `2026-09-18-gesheft-port-to-telnyx-zero-downtime.md`

- 2026-09-18 · LOOPCOM COMPUTER CONTROL — the Coworker runs Windows PROGRAMS now: ONE long-lived Local Worker (C# via Add-Type, nothing to install) does UI Automation + SendInput stamped with a signature the hook reads BACK (the 220 ms guess is gone) + capture; 20 `computer_windows_*`/app-launch tools press real buttons BY NAME with no cursor; cross-tool resource policy (a path refused for the file tool is refused for PowerShell, UIA and launch alike); PowerShell risk read from the SCRIPT; per-turn tool discovery. ✅ LIVE on this machine through the installed app: Notepad typed + File›Save As to disk, **Calculator 16,907 with ZERO positional clicks**, an acceptance app's text/radio/checkbox/combo/dialog/tab verified from the APP's own state file, the UIA-invisible purple control clicked after `computer_screen_look`, secrets refused on every road, injection summarized not obeyed, kill switch, **yield proven (person's mouse → `paused_by_person`)**, worker killed → restarted. ⛔⛔ THE RUN FOUND A BUG NOBODY HAD SEEN: one person with TWO computers shared ONE queue — the model was told machine A's tools and the file appeared on machine B (3 computers on this account); session is per COMPUTER now, a task stays where it started, STOP reaches all, reopening the app is how you redirect work. ⛔ Also: an interrupted task LOCKED the screen 4 h; ⛔ a `powershell -Command -` script whose block ends at EOF is SILENTLY DROPPED (wrap `& { }` + blank line). ⛔ Live score VARIES (13/17/17/18/18 of 20) — the MODEL's road varies; deterministic suites ran 10× clean (68/68, 41/41). ⏳ NO HUMAN has pressed Allow; Escape/UAC/multi-monitor unproven; not published to the fleet → `2026-09-18-loopcom-computer-control.md`

- 2026-09-17 · VITALPBX DASHBOARD BANNERS ✅ SUPPRESSED SERVER-SIDE 09-18 (blue "Unlock premium" ad = the canceled subscription; yellow "global limits" = OUR deliberate RTP widening, never lower it): panel's `showStartUp()` gated them on cookies `is_free_version`/`verify_global_limits` — IZZY EXPLICITLY AUTHORIZED one scoped PBX write (guardrail NOT lifted otherwise): the two `Cookies.get` reads in `01-pbx.min.js` are now `t="no",n="yes"` (−56 bytes, md5'd backup `/root/banner-fix-backup-20260918T031118Z`, live-serve verified); ⛔ a panel update reverts it → re-apply or use the cookie paste in the handoff; ⏳ Izzy's next login proves it (Ctrl+F5 once if cached) → `2026-09-17-vitalpbx-dashboard-banners.md`

- 2026-09-17 · YOSSIS WOOD WORKS ON CONNECT IVR (full migration, REAL-CALL-PROVEN: greeting + key 0; 845-827-9500 → profile cmu67d91000sers12b070ba2h, rollback = switch-to-pbx) + closed-Mon-Sep-21 Tiffany announcement BOOKED (custom/closed_monday_september_21_notice_4ff878, Sun 8am→Tue 12am ET, tick-run); ⛔⛔ PLANNER BUG FIXED `3049fd53`: a menu whose ONLY recording is the VitalPBX "instructions" message migrated SILENT (welcome ?? instructions now); ⛔ import-sourced prompts always show publish `missingAudio` — check the PBX file, not the flag; keys 1–5 stay PBX ring groups by design; ⏳ Sunday auto-start/Tuesday auto-stop + human call → `2026-09-17-yossis-woodworks-ivr-migration-closed-monday.md`

- 2026-09-17 · "CAN'T HEAR POLLY VOICES OR IVR RECORDINGS IN THE APP, other PC audio fine" — WINDOWS VOLUME MIXER HAD CHROME AT 0.00 (fixed live via CoreAudio SetMasterVolume 0→1; Studio player ran `playing` to `ended` unmuted the whole time), NOTHING CHANGED IN THE PRODUCT; server delivered all three (2× polly/preview 71/66 KB + prompt stream 116 KB, 200); client = real Chrome (desktop app stamps `Loopcom/<ver>` UA); ⛔⛔ I first called it the Aug-4 pipeline wedge off `readyState 0` probes in the EXTENSION's hidden un-activated tab and told him to restart Chrome — WRONG, a real click in his own tab played instantly; ⛔ check the mixer FIRST, extension-tab media probes prove nothing; ⏳ Izzy's ear on the replay → `2026-08-04-elevenlabs-didn-t-play-pipeline-hardening.md`

- 2026-09-17 · PLATFORM-WIDE AUDIO BREAKING UP (calls/hold/IVR/recordings) — DIAGNOSED READ-ONLY, nothing changed: the PBX's Contabo St. Louis uplink loses 5–24 % of packets toward VoIP.ms newyork1 (62/65 trunks + DID media) and Telocall `0001` (every tenant's primary outbound) on the Lumen transit path (loss first at 4.26.120.122), 0 % via Cogent (atlanta1/chicago1/us-central telocall/telnyx), 0 % from France and from Izzy's PC; proven by carrier RTCP (Telocall legs 4–5 % Transmit Lost live), `rtpStats` (trunk legs ≥1 % tx loss 1–4 %/day → 10 % Sep 6 → 49 % Sep 17, peak 11am–3pm ET, all trunk groups, first episode Aug 26–27) and the recordings (2.4–7.8 holes/min now vs 0–1.4 in Jul/Aug); PBX host + our code CLEAN (⛔ `/proc/stat` col 8 = steal; integer `rxLossPct` hides <1 %); side: `0001` registration Rejected (`server_uri :700` vs `:7000`), asterisk log keeps ONE day; ✅ Contabo ticket **16240435361** FILED 09-18 00:47Z from Izzy's panel (accounting proof: RTCP counted 49,894/3,546,852 lost Sep 17 vs ≈1,738 total guest-side drops since boot; panel diagnostic 'Failed' = our geo firewall blocking their probe, preempted; reboot-consent box was REQUIRED+ticked — any reboot off-hours only; ⛔ high-performance-server upsell does NOT help, same DC uplink); ⏳ remaining remedies Izzy's (`0001`→us-central / NY1→Chicago1 both halves / Telnyx) → `2026-09-17-platform-audio-breakup-lumen-transit-loss.md`

- 2026-09-17 · GESHEFT PAY LINE — FINAL FLOW + KEYED CARD + CHOOSE-THE-CARD-FIRST (rounds 3–5, api `af23a052` container-verified): known caller → "your account (1) or a different one (2)" → PIN (EVERY caller keys it, nothing enrolled) → 1 balance / 2 payment → cards on file by last four or press 9 for a new card (AGI collector, memory vault, charge straight through the POS); ⛔ no POS PIN → `36_no_pin_visit_store` + person; ⛔ Stephen = Polly neural; ⛔ deploys bind-failed on port 3004 → `ip_local_reserved_ports=3000-3010`; ⏳ a real keyed/picked-card charge → `2026-09-17-pay-line-final-flow.md` (history: `2026-09-17-pay-line-caller-id-rule.md`, `2026-09-17-pay-line-code-by-call-or-text.md`)
...
- 2026-09-18 · TELNYX 10DLC REGISTRATION — ✅ LOOPCOM LLC's OWN registration FILED 18:20Z (`cmu7aq8wx10avp41332itzy94`, brand `4b2001a0-b5cb…`, vetting pending; ⛔ its $24 CC-202609-00018 is billed to Loopcom itself — void; ⛔ billingSmsSender still VoIP.ms-hardwired, so pay links/OTP stay dead until it's provider-aware). ✅✅ GESHEFT = THE FIRST REAL FILING (09-18): his "denial" was Telnyx refusing the BRAND (`400 10015`, he picked "Publicly traded company"; nothing created); refiled as "Gesheft Supermarket Inc."/PRIVATE_PROFIT → brand VERIFIED in 13 min, campaign TCR `C5GLU0S` accepted, ALL SEVEN carriers approved by 15:05Z, $24 CC-202609-00017 OPEN, EIN destroyed; ⛔ number still VoIP.ms so not live; form now asks only LLC/Corporation/C corp + PUBLIC_PROFIT fails the checks (api+portal `ea3e11f7` container-verified). BUILT + DEPLOYED (api 34ff3aae, portal 9a04bb2c, browser-proven), live-proven to Ready-to-file, NO real filing yet: /admin/texting-registration → private link + Loopcom email ("to continue using SMS, fill out the 10DLC form") → locked pre-filled public form (customer edits only legal name/type/EIN/IRS address/website) → File with Telnyx runs unattended; ⛔ EIN AES-GCM token destroyed on verification; ⛔ charge = SEPARATE uncharged $24 one-time invoice (traced safe), monthly/marketing hidden; ⛔ own tables, never TenantSmsRegistration; ⛔ links on app.connectcomunications.com; ✅ 9 SMS customers’ drafts PREPARED; ✅ links OUT for Gesheft/Relax/Fixup (09-16) + TRUST (09-17, expires 10-17; URL only in the chat reply — a second Create REVOKES it; ⛔ curling the api GET stamps it "opened"), 5 still draft with no link (Izzy sends), ⛔ all their numbers are VoIP.ms; ✅ SWITCHER DEPLOYED `56879d0f`: a number that lands ACTIVE on Telnyx flips its texting VOIPMS→TELNYX by itself within 5 min + attaches to the approved campaign (723-1213 will flip Fri 9/18), ⏳ no real landing yet → `2026-09-16-telnyx-10dlc-registration-page.md`
...
- 2026-09-17 · YIDDISH LEARNING ENGINE + YIDDISH24 — **THE LOOP THAT LEARNS IS BUILT** (`f3c818b0`, api+portal deploy see summary): Izzy's target = the platform's Yiddish Whisper (ivrit.ai `yi-whisper-large-v3-turbo`, trained on only ~97 h read speech); sources Yiddish24 + voicemails + call recordings (owner-authorized, `YcRightsRecord` grants), ⛔ Yiddish Labs text never a label; money $5/day → training ≤ $10/day, "best free option", NOTHING PAID HAS RUN. Built: `transcribe`/`align` stages behind a backend interface (RunPod `everett` / `local` CPU / `none` → lawful SKIP), additive migration `20260917150000` (YcTranscript timing/words/logprobs), consent rung (CUSTOMER_PRIVATE eligible only with contentAllowed + GRANTED `training_export`), cost ledger in `/now`, gold-set routes + `/admin/yiddish-learning/gold` (nav `admin.yiddish_gold`, key `can_view_admin_yiddish_gold`, both toggle editors), `scripts/yiddish-finetune/` (dataset builder, LoRA `train.py`, RunPod cheap-GPU launcher `--max-cost-usd 10`, Kaggle FREE path, endpoint mgr), call-recording register/copy (⛔ PBX read-only), PC runner owns `segment,features,transcribe,align`. Tests 265/266 + 98 + 41 + 70. ⛔ MEASURED: this PC cannot label (44 s audio > 39 min); stored RunPod key DEAD (401), no Everett endpoint exists. ✅ FREE GPU POOL LIVE: Kaggle izzywein 2xT4 proven 30 h/wk (KGAT token on this PC) + Colab T4 + Lightning T4 ($4.82); ⛔ two office Chromes — select_browser first; ⏳ Izzy: Dell = labelling (bootstrap ready), ~1 h gold review. Waiting state by design: transcribe DEFERs until a backend + `integrator-budgets.sql`; voicemail fetch jobs parked. Earlier today: audio ON by owner permission, off-box runner, 24/7 download un-stalled → `2026-09-15-yiddish-learning-engine.md`
...
- 2026-09-18 · DESK PHONE WIZARD — R31 SECOND WALK: ⛔⛔ the R28 tenant-context helper checked for a UUID but tenant ids are CUIDs/slugs, so every switch was IGNORED ("still says no extensions") — fixed to the chat door's id alphabet, tests on real shapes; the settings page remounts the setup on tenant switch; Laybel now SEES THE ROBOT'S LAST LOOK (`observed.robot` → `GuideFacts.robot`, playbook: answer from it, never ask "what does the screen say" when the robot knows) → `2026-09-14-desk-phone-wizard-automatic-device-identification.md`
- 2026-09-18 · DESK PHONE WIZARD — R30 IZZY'S FIRST WALK: "stuck loading extensions" = the run was created on `connect-admin-tenant-v1` (tenant menu not on the customer; 0 extensions) and an empty list showed as "Loading…" — FIXED: super-admin with no customer picked gets no run + a "whose phones?" screen, extensions have honest loading/ready/empty/failed states with Laybel lines, the STICKER DEMONSTRATION (`MAC: 00-0B-82-1A-2B-3C`, last four lit, pairs/hyphens/colons, only 0–9 A–F), and typed O → 0 (`normalizeSticker`, the word MAC stripped because it is hex-shaped); ⛔ Landau has 1 Active extension in Connect (102 is PBX-only) → `2026-09-14-desk-phone-wizard-automatic-device-identification.md`
- 2026-09-18 · DESK PHONE WIZARD — R29 GRANDSTREAM ROBOT FAMILY (desktop `grandstreamWebRobot.ts`, cgi login/probe/provision-with-READ-BACK/reset; a GDMS-claimed phone's no-op write is refused `save_not_verified`, never called provisioned; `vendor` rides every web_* op; driver gates admit grandstream) + YIDDISH in the Laybel rail (`laybelLanguage.ts`/`ylClient.ts`: customer Yiddish → YL → English brain → fenced → YL Yiddish captions; avatar speaks English; English fallback) + THE MIC (`/laybel/hear` → YL transcriptions/sync, press-to-talk ≤30 s); ⏳ none proven live, desktop rc.21 see summary → `2026-09-14-desk-phone-wizard-automatic-device-identification.md`
- 2026-09-18 · DESK PHONE WIZARD — R28 SUPER-ADMIN RUNS IT FOR ANY CUSTOMER via the tenant switcher (Izzy: "at the tenant's location… from my account"): `effectiveUser.ts` — SUPER_ADMIN + `x-tenant-context` (UUID) → every desk-phone door (runs/extensions/folder/PnP pending/Laybel/managed panel) is THAT tenant's; other roles keep the token's tenant; ownership stays tenant-scoped; guided screen shows "Setting up phones for <Tenant>"; ⏳ not yet walked on a customer LAN → `2026-09-14-desk-phone-wizard-automatic-device-identification.md`
- 2026-09-18 · DESK PHONE WIZARD — R27 THE GUIDED SETUP IS BUILT (Izzy: "go"; additive, classic wizard one click away): `GuidedPhoneSetup.tsx` = ONE PHONE AT A TIME on the SAME run/driver — extension → phone cards keyed by LAST 4 OF THE MAC (typed to confirm) → per-MODEL reset recipe + picture (`resetRecipes.ts`, confidence-flagged: T-series PROVEN, GXP/HT/VVX/DECT documented, others inferred/generic) that ADVANCES ITSELF off a Yealink web_probe → robot timeline → green ONLY from per-device connectedNow AS the mapped ext → next; stuck wording from the server note; confirm click = the one authorize-reset. LAYBEL = `POST /desk-phones/runs/:id/laybel` (`laybelGuide.ts`: 13 scripted situations + gpt-4o-mini improviser behind a TRUTH FENCE that drops any connected-claim the facts don't back, password asks, URLs; key via resolveOpenAiKey — env is a placeholder) in a captions-always rail, Anam video optional; ⏳ no human walk yet → `2026-09-14-desk-phone-wizard-automatic-device-identification.md`
- 2026-09-18 · DESK PHONE WIZARD — R26 REDESIGN MOCKUPS ONLY (artifact Tty5aheU7hviHCMGi5vToG, awaiting Izzy): Izzy's directive = the robot road for EVERY phone (backend browser opens the phone UI, puts the link in, restarts; customer only factory-resets), ONE PHONE AT A TIME, confirm by LAST 4 OF THE MAC, LAYBEL LIVE in a permanent rail (Anam face + captions that never stop; brain = R23 OpenAI advisor + rules), per-MODEL factory-reset ILLUSTRATIONS (T42S + GXP2170 boards), reset screen advances itself off the network watch, green only from R25 per-device truth, STUCK screen = auto-filed RPS release + "I'll finish it myself"; ⛔ nothing built, no recipe/illustration library exists yet, Laybel wiring waits for the approved shape → `2026-09-14-desk-phone-wizard-automatic-device-identification.md`
- 2026-09-18 · DESK PHONE WIZARD — R25 ✅✅ GDMS ZERO-TOUCH PROVEN LIVE + "CONNECTED" IS PER-DEVICE TRUTH (api `828a07a8` deployed+container-verified): factory-reset .172 (c074ad8c605f) self-provisioned THROUGH the GDMS redirect and registered as T21_101 with NO wizard — GDMS reads it online+synchronized (⛔ creds via resolveGdmsCredentials(db), never env); .171 not in GDMS = why it sat; AND the wizard's status lied both ways (Izzy: "never say connected without it actually being connected… real data, not fake") — the per-ENDPOINT mirror let 2 phones on ext 101 overwrite each other (AOR max_contacts=5), ✅ NEW `PbxContactRegistration` keyed (endpoint, phone's LAN IP from x-ast-orig-host) + pure `phoneRegistrationTruth()` gates EVERY green (own contact decides + names registeredAsExt; extension held by ANOTHER device = refused; green needs registered AS the mapped ext; cold mirror falls back extension-level); migration 20260918040000; 11 new tests replay the live run, 260 desk-phone green; live rows within seconds of cutover; ⛔ deploy = `bash scripts/deploy-direct.sh` from `/opt/connectcomms/app`; ⏳ Izzy hasn't re-opened the wizard; robot-reboot-on-stall designed, not wired → `2026-09-14-desk-phone-wizard-automatic-device-identification.md`
- 2026-09-18 · DESK PHONE WIZARD — R24 GDMS REDIRECT BUILT (Izzy: "Build the GDMS redirect for Grandstream"): ⛔ GDMS has NO template-level redirect API (spec-proven, 41 endpoints) — per-device `device/config/xml` IS the redirect (sets P237→our folder); NEW `gdmsRedirect.ts` TARGET FENCE (private/VPN/loopback P237 or P47 → refused BEFORE the cloud — the proven 10.8.0.1 flap bug; no-P237 refused; look-alike hostnames refused) in pushConfig AND /prepare + `deliverRedirect` honest states (delivered / sent_applying / queued_offline — the silent-queue trap now SAYS "waiting for the phone to check in" / not_claimed / refused) via GDMS isSynchronized; 381 api tests (1 pre-existing fail), tsc clean; ⏳ factory-Grandstream hands-off register unproven → `2026-09-14-desk-phone-wizard-automatic-device-identification.md`
- 2026-09-17 · DESK PHONE WIZARD — R23 THE HIDDEN ROBOT (Izzy: a robot + hidden browser in the wizard, OpenAI agent runs it and improvises, customer only factory-resets): ✅ LIVE PROOF — headless-Chrome robot logged into the factory T42S admin/admin, wrote our phoneprov folder, REGISTERED as T21_101 in 65 s; BUILT — desktop `phoneWebRobot.ts` 4 fenced ops (caged to the phone IP, only types a Loopcom folder), driver falls FORWARD to it when PnP can't reach the office PC, unknown screens → `/robot-advise` → agent `phone_web_robot` (OpenAI) + PHONE_ROBOT_PLAYBOOK, dead "locked" misread, auto Yealink MAC-removal filing (Admin→Integrations card, one-time cookie); ⛔⛔ URL fence in BOTH desktop+api, hardened for bare hostnames + server-named fields; rc.19; ⏳ not deployed, wizard-e2e/real-handset/real-filing unproven → `2026-09-14-desk-phone-wizard-automatic-device-identification.md`
- 2026-09-17 · DESK PHONE WIZARD — R22 "not one successful register": chain MEASURED end to end (PnP resident hears the LAN; cfg = the working T53W's; ZERO cfg fetches by the T42S in 13 days; RPS live) → product correct, Izzy's T42S = previous provider's lock + RPS claimed by another org (800004): hand reset + Yealink MAC-removal ticket, a factory-fresh Yealink is the acceptance test; ✅ BUILT+DEPLOYED: office wizard claims every Yealink into OUR RPS (redirect = tenant phoneprov folder, ONE writer, conflict = honest note, "Zero-touch on" chip) + scan-page decoder CANARY + mode telemetry + deploy-portal FAILS on a CSP without wasm-unsafe-eval; R1–R21 unchanged → `2026-09-14-desk-phone-wizard-automatic-device-identification.md`
