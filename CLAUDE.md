...
- 2026-09-17 · "CAN'T HEAR POLLY VOICES OR IVR RECORDINGS IN THE APP, other PC audio fine" — IZZY'S CHROME AGAIN (2nd time; 08-04 was ElevenLabs), NOTHING CHANGED IN THE PRODUCT: nginx delivered all three (2× polly/preview 71/66 KB + prompt stream 116 KB, 200); in his Chrome a plain fetch of an MP3 = 39 KB/631 ms while the `<audio>` for the same URL never even sends the request (readyState 0, one resource entry) — browser-wide, new renderer too; ⛔ extension clicks grant NO user activation (play()/resume() results are inconclusive); ⛔ killing Chrome's `audio.mojom.AudioService` helper did NOT clear it; fix = full Chrome quit + reopen (⏳ awaiting Izzy) → `2026-08-04-elevenlabs-didn-t-play-pipeline-hardening.md`
