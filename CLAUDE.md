...
- 2026-09-17 · GESHEFT PAY LINE — FINAL FLOW + PAY WITH A KEYED CARD + CHOOSE THE CARD FIRST (round 5 api `af23a052` container-verified, final-flow + card-door proofs green on it; after the amount is confirmed the line reads the cards on file by last four — "to use the card ending in 9 6 0 3 press 1 … to enter a new card press 9" — and charges THE PICKED id, never the first card; a decline (on file or keyed) says "declined" and re-offers the cards + a new card, amount kept; Izzy's round-4 call had charged the card on file at once and a decline fell into the amount-retry branch; prompts 48–50, en-male 79; tests **238/238** (a 500-session stress asserts the charged id is ALWAYS the picked one); round 4 api `d2bf48ee` + `f238b0bf` container-verified — ⛔ the first deploy's card door 401'd the AGI with the CORRECT secret because it was not on the JWT public-route bypass, fixed + guarded; live proof: door fail-closed, invalid refused, valid vaulted with 0 digits persisted/logged, the INSTALLED AGI driven through a fake pipe to the live door with 0 PAN in the PBX log; tests **229/229**, payLineCard 19 NEW incl. a 500-session stress + a secrecy sweep; confirm says "3 = pay with a different card"; no card on file → offer to key one; number/expiry/CVV/ZIP collected by an **AGI** (`connect-pay-card.py`, pbx agi-bin) because every dialplan step is LOGGED with substituted args, posted once to `/internal/supermarket/pay-ivr/card`, held in a process-memory vault, charged **once** = inline card on the POS `/charges` or **saved** = `POST /cards` then charged by id; register fields `CardNumber/ExpMonth/ExpYear(0–99)/CVV/ZipCode` read off its validator; only `cardLast4` ever persists; ⛔ PCI transmission scope accepted by Izzy ("Go, build it."); prompts 39–47 (en-male 76); ⏳ a real keyed charge is the first proof of the inline-card shape; final flow round 3 api `25ef3d12` shipped as tip `64746df2`, container-verified, live proof ×5 + Izzy's own calls reached 3 accounts with the PIN; ⛔ two deploys FAILED "bind 127.0.0.1:3004 address already in use" = an nginx keepalive's ephemeral SOURCE port on the candidate port (perf tuning lowered the range to 1024) → `ip_local_reserved_ports=3000-3010` set + persisted): known caller hears "to pay or hear a balance on the account for the number you are calling from press 1, for a different account press 2" (`37_which_account`); 1 → PIN, 2 → phone number (`38_enter_phone`) → PIN; unknown caller → `13_not_recognized` → number → PIN; EVERY caller keys the PIN, nothing enrolled, vault never read; then 1 balance / 2 payment; ⛔⛔ no PIN in the POS (register refuses balance AND charge) → silent probe catches it first → `36_no_pin_visit_store` ("visit the store to set one up") + person, `status no_pin` (Izzy's cell 1001021 is one; his 3064 line 3762 has a PIN + Visa); ⛔ the same day's two earlier designs (matched = never a PIN + one-time code by call/text; then the "own account unservable → ask for another account" redirect) were built, deployed and REMOVED on Izzy's own calls — conf `Originate` + code contexts gone from repo AND PBX (`.bak.paycode-removed.20260917T204449Z`), guard test forbids any Originate/Dial, retired WAVs 23–35 stay on the PBX (en-male 67); ⛔ Stephen = Polly NEURAL, not ElevenLabs; the line charges STRAIGHT THROUGH THE POS (Sola only on the Orders desk's new-card capture); tests **208/208** (payLineStress 2,000-account matrix: served ⇒ keyed the PIN, vault touched 0×); ⏳ a human hearing 37/38/36, a keyed PIN reaching the menu, a real charge → `2026-09-17-pay-line-final-flow.md` (history, register facts still true: `2026-09-17-pay-line-caller-id-rule.md`, `2026-09-17-pay-line-code-by-call-or-text.md`)
