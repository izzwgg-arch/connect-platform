...
- 2026-09-16 · SIGN-UP WIZARD SWITCHED TO TELNYX (live, switch=telnyx): search/buy/E911/CNAM/real Telnyx port filing + landing sweep + shared trunk 183; ⛔⛔ 5 live-only bugs fixed (autosave WIPED the carrier stamp → submit pins it; messaging profile needs /messaging PATCH; shared-trunk builds had NO Main DID dispatch → panel saveTenant + applyAndRebake(Main); interrupted builds could not resume + stale REST directory → read ombu_tenants); ✅ real run: 845-777-4807 bought, T143 built, ACTIVE (card NOT charged — test invoice marked paid); ⛔ no Telnyx stock in 212/718/347/646/917/332/201; + evening: 911 postal form (State Route 17M/Ste C) + owner SMS alert + auto/admin retry + FastPort; ⛔ Telnyx 911 blocked until Izzy accepts Emergency ToS; ⏳ 723-1213 port prepped in wizard awaiting Izzy signature+bill+pay, human call, 10DLC EIN → `2026-09-16-telnyx-onboarding-wizard.md`

- 2026-09-16 · TELNYX 10DLC REGISTRATION — MOCKUPS V3 AWAITING IZZY, NOTHING BUILT (artifact UjnMidL2iSzitYgutQCvbk V3): private link → Loopcom email → pre-filled form (⛔⛔ customer SEES system wording, EDITS only legal name/EIN/type/IRS address/website) → admin presses File with Telnyx; ✅ EIN TOKENIZED, $24 up-front on the customer's invoice; ⛔ monthly fee + marketing DEFERRED, never shown; ⛔ privacy policy NOT a hard registry field but TCR says required + must be the brand's own (plan: Loopcom-hosted in customer's legal name, unproven); build to Telnyx's OpenAPI spec not its docs → `2026-09-16-telnyx-10dlc-registration-page.md`
