...
- 2026-09-15 · ONBOARDING WELCOME EMAIL → GOOGLE PLAY BADGE: MOCKUP ONLY, nothing built/edited/sent (artifact YSi8QdS7P9NDFBUZJfEjWH, `docs/mockups/onboarding-email/`) — the two email renders are the REAL `welcomeCreatePasswordEmail()` output, only the Android block swapped: outlined "Download Loopcom for Android" → Google's official badge 180×70 → `play.google.com/store/apps/details?id=com.connectcommunications.mobile`; ⛔ TWO callers share the template via `getAndroidApkUrlForInviteEmail()` so both move together, ⛔ badge must be OUR absolute https asset resolved at call time like `brandLogoUrl()` (never a caller input), APK download page stays; ⏳ 2 decisions open with Izzy (badge-only vs + text fallback for images-off Outlook; hard-code vs keep `ANDROID_APK_DOWNLOAD_PAGE_URL`) → `2026-09-15-onboarding-welcome-email-google-play-badge.md`

- 2026-09-15 · COWORKER WORKSPACE BUILT + DEPLOYED (`7b26c388`): the IDE-style chat — live plain-English steps, asks questions back, Stop, voice (Yiddish Labs + GPT), ANY file read on the server, folders + git projects, full page `/coworker` (key `can_view_workspace_coworker`, no default bucket) with Settings; agent rebuilt + portal deployed + desktop rc.17 BUILT (⛔ not installed, not published); ⛔⛔ the chat page is HOSTED so it is never the boundary — approvals stay in the desktop's own approval window and raising access needs a NATIVE dialog; ⏳ nobody has run a real task with the hands yet → `2026-09-15-coworker-workspace-built.md`
...
- 2026-09-15 · COWORKER IDE REDESIGN (the approved mockup — SUPERSEDED BY THE BUILD ABOVE): live mockup v1 (artifact 9rkPBPFRp6hqQJcYCaSDXG) — bubble chat with live plain-English steps + in-chat approvals, full page with plan/live-window/settings; NOT BUILT, awaiting Izzy; ⛔ in-chat approval vs today's local approval window needs a security call first → `2026-09-15-coworker-ide-redesign-mockup.md`

- 2026-09-15 · COWORKER SCREEN CONTROL + ADMIN POWERSHELL: mockup APPROVED → BUILT (Phase 1 + Phase 2 vision), OFF by default (`coworkerScreenControlEnabled` tray toggle). 10 `computer_screen_*` tools (buttons-first UIA click-by-name, cursor fallback, + `computer_screen_look` = model PIXEL-VISION) + elevated PowerShell, gated by `desktop.active` (already NEVER_AUTO). ⛔ GATES + VISION TRANSPORT PROVEN by test (44: 32 desktop incl. 500-action ask-once stress + 11 agent cross-provider image transport — Anthropic image block / OpenAI input_image / image-aware boundContent; tsc clean) — the LIVE cursor/screen/UAC/vision-through-a-provider leg is UNPROVEN, needs a human on a real Windows screen; agent transport deployable now. Handoff AGENT_HANDOFF_COWORKER_SCREEN_CONTROL_2026-09-15.md → `2026-09-15-coworker-desktop-control-research.md`
...
- 2026-09-15 · FCC BDC FILING (late, June-30-2026 data): 102 iVoIP subs / 8 NY tracts entered + valid, all business, pure OTT; cert hard-blocked by "Missing Form499 ID" — BDC helpdesk ANSWERED #63588: their 499 list refreshes ~monthly, so 839208 just has to appear, then ratio-warning note + Izzy certifies. ✅ THE FILING IS GENERATED NOW — `apps/api/scripts/bdc-voice-subscription-export.ts` + `docs/regulatory/bdc-tenant-tracts.json`, PROVEN to reproduce June exactly (8 tracts/102) and REFUSES to emit if any tenant with extensions is unmapped. ⛔⛔ FCC 499 XML API is DEAD (errors on its own sample) — no watcher for 839208 possible; CPNI/ECFS API is GET-only; 499-A/RMD/CVAA/DC-agent manual → `2026-09-15-fcc-bdc-filing-late-notice.md`
