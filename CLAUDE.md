...
- 2026-09-16 · OCTOBER 2026 CHARGE PROJECTION (read-only): **$1,836.04** across 17 autopay tenants (all default cards ACTIVE, none expiring before Nov), realistically **$1,781.04** — Secro's mis-mapped Sola link still blocks Oct 6; ~$2,153 counting Yossis $206.96 manual + Nexus $65 and Fixup $45 on Sola's own rail. Amounts are the ENGINE's own `/admin/billing/platform/tenants/:id/invoice-preview?periodMonth=10` run per tenant inside `app-api-1` (⛔ api listens on 3001, and `jsonwebtoken` is not installed — hand-roll the HS256 token), not hand math. ⛔⛔ FOUND: inii mini + Matamim + TYH lost ALL of September to a 17-hour paid-period overlap (`billing.autopay_skipped_period_already_paid`, ~$128, never caught up) and RSBK auto-charges $13.91 on Oct 20 with 4 ACTIVE billable extensions because `billingQuantityOverrides.extensions` is pinned manual 0 → `2026-09-16-october-2026-charge-projection.md`

- 2026-09-16 · RELAX TIRES exts 102 Alex Silva + 103 Felix Nieto (T25) — both billable + live (`pjsip show endpoints`), both USER_INVITE emails SENT; ⛔ brief's `Alex@relaxtores.com` is NXDOMAIN → used `Alex.relaxtires@gmail.com` (check invite domains in DNS); ✅ Sep 26 invoice CC-202609-00004 RE-ISSUED $45 → **$105** via `PUT /line-items` (qty 1→3, period untouched, backup `/root/relax-invoice-backup-20260916.json`) and re-sent to the customer, ⏳ charge is Sep 26. ⛔⛔ THE BIG ONE: the panel now refuses EVERY app/WebRTC device (`vitxi_clients.max_reached`, and freeing a slot does NOT reopen it) **and `/mirror/extension-add` has never run on prod** — it was proven on the CLONE as MySQL root, while `mirror-grants-20260819.sql` granted tenant-create tables only → `connect_route_helper` gets `INSERT denied on ombu_extensions`. Fix written, **needs Izzy's Run button**: `scripts/pbx/mirror/mirror-extension-grants-20260916.sql` (INSERT on 7 tables). Until then EVERY new extension platform-wide is desk-phone-only, incl. Felix → `2026-09-16-relax-tires-two-new-extensions.md`
...
- 2026-09-16 · Browser Companion: local rc.18 hybrid prefers explicit paired Loopcom MV3 for new tasks, otherwise isolated Playwright; task engines pin. Branded icon/package/bridge/security/hybrid/build checks pass, but supported Windows control offered no native launcher and refused `chrome://extensions`: not installed/paired or live-provider accepted. Chrome 139 unpacked load must use that UI; no distribution/recovery/stress release claim → `2026-09-14-browser-companion.md`
