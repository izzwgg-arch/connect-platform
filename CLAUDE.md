...
- 2026-09-18 · LOOPCOM WORKS (TrimPro → Loopcom product, `Loopcom works/` in THIS repo, branch `works/loopcom-works`, worktree `C:\dev\projects\c2-works`) — INVENTORY DONE (8 Sonnet audits under `docs/ai-context/loopcom-works-audit/`: 787 TrimPro strings, 3,990 hard-coded colours + dead dark mode, **13 CRITICAL security findings** incl. unauth’d set-password + unsigned Sola payment webhook + 6 cross-tenant IDOR writes, real mobile app = `apps/mobile/` Expo 54), ARCHITECTURE DECIDED (immutable-id identity map, 60-s single-use SSO code exchanged server-to-server, seamless nav not iframe, `/api/v1/works/*` HMAC service API, NEW outbound signed-webhook dispatcher for screen pop, agent gets a third `resolveIdentity()` branch + `worksTools.ts` — standalone orgs use `works:<orgId>` since `AgentConversation.tenantId` has no FK, Docker Compose deploy per `infra/community/`), **MOCKUPS PUBLISHED, awaiting Izzy: https://claude.ai/artifact/LZvf79qD8f5aF3nfQ55nDy** (30 boards light+dark, repo copy `docs/mockups/loopcom-works/mockups-v1.html`); repo hygiene done (nested 3.8 GB `.git` moved to `C:\dev\projects\_backups\`, junk gitignored, legacy DB password scrubbed); ⛔ `prisma/migrations/*/migration.sql` is gitignored in the copy; ⛔ TrimPro prod `154.12.235.86` untouched/out of scope; ⏳ NOTHING BUILT, six decisions are Izzy’s (§5) → `2026-09-18-loopcom-works-audit-architecture-mockups.md`
