...
NOW="$(date +%s)"
EMAIL="smokepbx${NOW}@connectcomunications.com"
fail(){ echo "[smoke-pbx] FAIL: $*" >&2; exit 1; }
...
signup="$(api POST /auth/signup "" "{\"tenantName\":\"Smoke PBX ${NOW}\",\"email\":\"${EMAIL}\",\"password\":\"${PASSWORD}\"}")"
token="$(echo "$signup" | jq -r '.token // empty')"; [[ -n "$token" ]] || fail "token"
status="$(api GET /admin/pbx/events/status "$token")"
echo "$status" | jq -e '. != null' >/dev/null || fail "pbx status shape"
tenantRes="$(api GET /admin/pbx/tenants "$token")"
echo "$tenantRes" | jq -e '.tenants != null or .error != null' >/dev/null || fail "tenant list shape"
echo "[smoke-pbx] PASS"
