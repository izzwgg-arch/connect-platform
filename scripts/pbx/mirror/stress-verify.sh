#!/usr/bin/env bash
# Verify all 10 stress tenants on the PBX. Reads /root/stress-manifest.json.
set -uo pipefail
...
mf=[json.loads(l) for l in open("/root/stress-manifest.json") if l.strip()]
ok=True
...
for m in mf:
    ...
    ep = eps.count(f"T{t}_1")  # counts T<t>_10x and T<t>_10x_1 endpoint lines loosely
    ...
    line_ok = files==17 and epd==10 and exts=="5" and devs=="10" and route>=1 and cos>=1
    ok = ok and line_ok
    print(f"{slug} t={t}: files={files}/17 endpoints={epd}/10 exts={exts}/5 devices={devs}/10 vm_users={vm} hints={hint_n} inbound={route} cos={'ok' if cos else 'MISSING'} astdb_keys~{astdb} -> {'PASS' if line_ok else 'FAIL'}")
