...
cd "$REPO_ROOT"
run_compose "smoke-v1.3.2.sh:compose-up" "PBX_SIMULATE=true VOICE_SIMULATE=true MOBILE_PUSH_SIMULATE=true PBX_WEBHOOK_VERIFY_MODE=token PBX_WEBHOOK_TOKEN=\"$WEBHOOK_TOKEN\" docker compose -f docker-compose.app.yml up -d api worker >/dev/null"
