...
# when the diff touches no realtime-relevant paths.
#
...
if [[ "${DEPLOY_DRY_RUN:-0}" == "1" ]]; then
  ...
  log "Would next: pnpm install if needed, docker compose build/up ${SERVICE}, GET :3002/health"
...
if ! deploy_common_wait_http_ok "http://127.0.0.1:3002/health" 30 2; then
  rollback
