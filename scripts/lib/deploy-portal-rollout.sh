...
#
# DEPLOY_PORTAL_PUBLIC_VERIFY_RESOLVE_LOCAL=1 maps the hostname to 127.0.0.1 so the probe does not
...
# every rollout burned its whole probe budget on a connection that could never be made and then
# ROLLED A PERFECTLY HEALTHY DEPLOY BACK. So the loopback path is PREFERRED, never MANDATORY: when
# it cannot connect we fall through to ordinary DNS inside the same attempt, and only fail when
# BOTH paths fail. See scripts/lib/deploy-api-rollout.sh for the same rule on the api side.
deploy_portal_rollout_probe_resolve_specs() {
  local url="$1"
...
deploy_portal_rollout_wait_ready() {
  ...
  local specs=()
  while IFS= read -r _spec; do specs+=("$_spec"); done < <(deploy_portal_rollout_probe_resolve_specs "$url")
  ...
  while [[ "$n" -lt "$attempts" ]]; do
    for spec in "${specs[@]}"; do
      ...
      if curl "${curl_tls[@]}" "${curl_resolve[@]}" -fsS --connect-timeout 2 --max-time 15 "$url" >/dev/null 2>&1; then
        if [[ "${#specs[@]}" -gt 1 ]]; then
          deploy_common_log "[deploy-portal-rollout] public verify ok via ${spec:-dns} url=${url}"
        fi
        return 0
      fi
    done
  ...
  # The api side has logged this since it shipped; the portal side used to fail SILENTLY, so a
  # rolled-back portal deploy said only "not ready after cutover" with no code to read.
  if [[ "$url" =~ ^https:// ]]; then
    ...
    deploy_common_log "[deploy-portal-rollout] public verify probe failed: url=${url}${codes} resolve_local=${DEPLOY_PORTAL_PUBLIC_VERIFY_RESOLVE_LOCAL:-0} tls_insecure=${DEPLOY_PORTAL_PUBLIC_VERIFY_TLS_INSECURE:-0} (http_code=000 is a CONNECTION failure, not a bad HTTP status)"
  fi
  return 1
}
...
deploy_portal_rollout_run() {
  ...
  if [[ -n "$verify_url" ]]; then
    if [[ "${DEPLOY_PORTAL_PUBLIC_VERIFY_RESOLVE_LOCAL:-0}" == "1" ]]; then
      deploy_common_log "[deploy-portal-rollout] public verify prefers loopback SNI (DEPLOY_PORTAL_PUBLIC_VERIFY_RESOLVE_LOCAL=1 curl --resolve host:443:127.0.0.1); falls back to ordinary DNS if nothing listens on 127.0.0.1:443"
    fi
...
Blue/green portal deploy (when DEPLOY_PORTAL_BLUEGREEN=1 and upstream include exists):
  ...
  6. (optional) DEPLOY_PORTAL_PUBLIC_VERIFY_URL — HTTPS after cutover; DEPLOY_PORTAL_PUBLIC_VERIFY_RESOLVE_LOCAL=1 PREFERS
     curl --resolve host:443:127.0.0.1 (origins where the hairpin public URL gets nginx 403) and FALLS BACK to ordinary DNS
     when nothing listens on 127.0.0.1:443. Proof: bash scripts/lib/deploy-rollout-probe.test.sh
