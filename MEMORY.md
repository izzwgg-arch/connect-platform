...
Talk to Laybel (2026-09-15): deployed by successful blue/green portal job `c2ba7701…` (6:16:54 PM, stage done, commit `7d93d23a8`) after dry-run `ad871db3…`; local ancestry confirms that revision includes Laybel. Compilation/typecheck/static generation passed and `/ready` returned 200. ⛔ Not container-verified: canonical Linux SSH was unavailable and Deploy Center log UI timed out; future verification must read the final `done 7d93d23a806c2d14b48c2c0196b66e1bc7187293` line and unique source in the running container. Preserve the existing `FloatingAssistant` appearance exactly; the only added choice is `Talk to Laybel`, an opt-in push-to-talk mode for the same Assistant. It relies only on existing authenticated transcription/conversation paths, adds `channel: "voice"`, uses browser-local speech where available, and retains transcript/typed fallback. No avatar, video, LiveKit/WebSocket, provider, database, backend route, policy or PBX change. Do not revive the prior LiveKit/Anam/portrait exploration without a new explicit request. Verification/boundary: `docs/ai-context/AGENT_HANDOFF_FACE_TO_FACE_AI_SUPPORT_2026-09-15.md`.
