...
Talk to Laybel (2026-09-15): implementation is complete and pending normal portal rollout. Preserve the existing Loopcom `FloatingAssistant` appearance exactly; the only added choice is `Talk to Laybel`, an opt-in push-to-talk mode for the same Assistant. It relies only on the existing authenticated transcription and Assistant conversation paths, adds `channel: "voice"`, uses local browser speech synthesis for replies when available, and retains the visible transcript/typed composer fallback. No avatar, video, LiveKit/WebSocket, provider, database, backend route, policy or PBX change. Do not revive the prior LiveKit/Anam/portrait exploration without a new explicit request. Verification/boundary: `docs/ai-context/AGENT_HANDOFF_FACE_TO_FACE_AI_SUPPORT_2026-09-15.md`.
