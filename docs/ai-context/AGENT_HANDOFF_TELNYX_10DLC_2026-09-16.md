...
## 000f. 2026-09-18 18:20Z — Loopcom LLC's own registration (the platform number)
Izzy: *"Create a 10DLC for the Loopcom number for Loopcom."* Done through the deployed api with the §000c HS256 recipe (`/tmp/l.js` inside `app-api-1`): `POST /admin/texting-registration {tenantId: cmqzfigij4bt0mw13u2ulpd0t}` → `cmu7aq8wx10avp41332itzy94` (⛔ there is no `/prefill/:tenantId` GET — 404; prefill happens inside create and came out as "Connect Communications", ezra@connectcomunications.com, contact "e e"); `PATCH …/business` with markSubmitted overrode displayName/email/phone and set legalName Loopcom LLC, PRIVATE_PROFIT, EIN (from memory `loopcom-fcc-frn-and-federal-registrations`, passed as an env var, never argv), the postal-form office address, website; checks all pass (the one TENANT number is already TELNYX so even the `numbers` warn is absent); `POST …/file` → `brand_review`, brand `4b2001a0-b5cb-40a0-720f-5dc56dad6f07`, charge CC-202609-00018 (⛔ billed to Loopcom itself — void). The point: once live, 723-1213 can send to wireless carriers again (40010 today). What it does NOT fix: `billingSmsSender.ts` is hard-wired to VoIP.ms from 723-1213 (`invalid_did`) — pay links/OTP need the sender made provider-aware (TELNYX row → TelnyxSmsProvider) or the from-number moved.
