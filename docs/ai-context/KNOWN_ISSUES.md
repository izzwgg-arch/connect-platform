...
- **Fixed 2026-05-18 — Tax estimate showed 0/0 for flat-rate tenants.** `buildBillingInvoicePreviewWithLoadedSettings` generated no extension line if `extensionBillableCount = 0` (new/migrating tenant with no active extensions); portal fell back to a calculation path that didn't account for flat-rate or toll-free pricing. Fix: API now exposes `taxableSubtotalCents` in `BillingInvoicePreview`; portal estimate prefers this field.
- **Fixed 2026-05-18 — Add-card showed generic "not configured" when gateway enabled but iFields key missing.** UI now distinguishes the two failure modes and guides to the exact missing field.
...
- **Fixed 2026-05-19 — Invoice PDF email attachment + broken JWT PDF links.** Invoice/receipt emails previously linked to `GET /billing/platform/invoices/:id/pdf`, which requires login — recipients saw broken downloads. **Fix:** `sendEmailJobNow` in `apps/api/src/server.ts` attaches the generated PDF at send time (`billingEmailAttachments.ts`); templates use `billingInvoiceEmailMarker` instead of API links. Admin **Download PDF** uses `GET /admin/billing/invoices/:id/pdf`; SUPER_ADMIN cross-tenant PDF via `billingInvoicePdfAccess.ts`. No `EmailJob` schema migration.
...
- **Deploy metadata can claim a direct deploy target shipped while containers are stale (observed 2026-06-06).** During the CRM rollout, direct deploy dry-runs advanced `/opt/connectcomms/app` to the target SHA; immediate real `api`/`portal` deploys then printed `skip=no_changes`. Container grep proved the API image did not contain the new `crmSmsTemplate` code. Recovery used the approved `scripts/deploy-direct.sh` path with a temporary `DEPLOY_QUEUE_STATE_DIR` whose `last-deployed/<service>.sha` pointed at the pre-change SHA, forcing normal change detection, migrations, blue/green rollout, and health gates. Always verify the running container (`grep` unique API line, portal `.build-commit`, `/ready`) after a dry-run + deploy pair. See `DEBUGGING.md` § Deploy queue/direct deploy verification. **Hardened 2026-06-10:** `apps/api/Dockerfile` now bakes `/app/.build-commit` (like portal), `scripts/deploy-api.sh` reads it from the running `app-api-1` as the `OLD_HEAD` fallback when no `DEPLOY_QUEUE_STATE_DIR` marker exists (so a dry-run-advanced clone can no longer fake `no_changes`), and a new `verify` stage rolls back + fails if the live container's commit ≠ target after rollout. The portal path already had this guard via its container `.build-commit` fallback.
- **Fixed 2026-05-13 — Connect Desktop background polls hammering `app-api-1` CPU.** `DesktopNotificationsBridge` polled every **30 s** with a **hard-coded** `GET /voice/voicemail?folder=inbox&page=1&pageSize=10` that omitted **`tenantId`**, so **`SUPER_ADMIN`** sessions hit **`400 tenant_required`** (see `GET /voice/voicemail` in `server.ts`). The same loop called **`loadSmsThreads("GLOBAL")`**, which always **`GET /admin/sms/provider-health`** first — even while the user sat on unrelated pages (nginx showed **Referer: `/billing/invoices`**). **Fix:** `apps/portal/lib/desktopNotificationPoll.ts` (valid probe path + **independent exponential backoff** per SMS vs voicemail), `fetchTenantSmsInboxThreads()` in `apps/portal/services/platformData.ts`, and `apps/portal/components/DesktopNotificationsBridge.tsx` (tenant SMS inbox only for notifications; voicemail probe gated on `can_view_workspace_voicemail`). Diagnosis used **nginx `access.log`** + **`connect_api_request_duration_seconds`** from **`GET /metrics`** — see **`DEBUGGING.md`** § *API CPU — nginx + Prometheus (no env profiler)*.
 - **Fixed 2026-05-24 — Chat thread list heavy self-heal + unread N+1.** `GET /chat/threads` previously upserted every tenant user into the default group on each request and computed unread per-thread. Now the route performs a cheap ensure (create-if-missing, upsert current user only) and aggregates unread with a single SQL group-by over visible threads.
 - **Fixed 2026-05-24 — Chat messages lacked pagination/delta params.** `GET /chat/threads/:threadId/messages` now supports `?before`, `?after`/`?since`, and `?limit` (cap 200). Default oldest-first remains unchanged; `before` returns a chronological page (server reverses internally).
 - **Guidance 2026-05-24 — SMS media link base missing.** When `PUBLIC_API_BASE_URL` (or `PORTAL_PUBLIC_URL`) is unset and an SMS send with attachments must fall back to signed links (non-MMS DIDs), the API returns `MEDIA_LINK_BASE_UNAVAILABLE`. Mobile surfaces an inline banner with the admin step. Avoid sending SMS media until the base URL is configured.
- **Limitation — CRM Forms Phase 1 stores structured submissions, not flattened PDFs.** `/crm/forms` supports reusable PDF templates, manual fields, secure hashed-token public links, and `CrmFormSubmission.submittedData`. `completedPdfStorageKey` remains nullable and completed PDF overlay/flattening is pending. Do not represent Phase 1 as producing a signed PDF artifact; the durable completed record is the structured submission plus timeline.
...
- **Voicemail greeting recording flow.** Recently changed:
  ...
  - **RESOLVED — VERIFIED WORKING by Izzy 2026-08-04** (upload + Call-to-Record,
    T21 ext 101, desktop + Android ringing simultaneously). Three further live
    failure layers were found and fixed on top of the phases below — full story
    in `docs/ai-context/AGENT_HANDOFF_VM_GREETING_2026-08-04.md`:
    api `707820cb` (instant-originate: skip the wake push + 12s wait when the
    requesting client's validated `callerSipEndpoint` is already Avail — the
    wake churned the shared AOR mid-ring and broke answering; wake fan-out now
    bounded to devices seen ≤30 days), helper v2026.08.04.2 / commit `1f216a80`
    (dispatch dialplan expands `PJSIP_DIAL_CONTACTS(base)` + `(base_1)` at dial
    time — `Dial(PJSIP/<endpoint>)` created ONE channel per endpoint so only one
    device of a multi-contact AOR ever rang; AstDB `connect_vm_dial` is fallback
    only), api `b6034b7b` (⛔ the Android ring screen is PUSH-DRIVEN — a bare
    SIP INVITE renders no UI; the synthetic `INCOMING_CALL` push must be sent on
    EVERY vm-record path; only the WAKE push is skipped).
...
- **⛔ Filtered/proxied internet is the NORM across Connect's user base** (confirmed by Izzy
  2026-08-03), and it has **two** failure modes. (1) *Hard block* — filters allow 80/443 only,
  so WSS on **8089** never reaches the PBX: zero trace in Asterisk logs while the portal works
  fine over 443 (Trimpro ext 105, 2026-07-28). (2) *Rotating egress* — the filter permits the
  connection but load-balances it across an address pool, repeatedly stranding the socket:
  Luxure ext 101 on 2026-08-02 registered **128 of 129 times through one filter**
  (Cologuard `192.157.80.0/20`, Old Bridge NJ, no rDNS) across six addresses, with exactly
  **one** direct session over the real ISP. **Diagnostic: `whois` the device's
  `PbxEndpointRegistrationEvent.contactUri` IP before blaming Wi-Fi** — datacenter/colo block
  = proxy, residential = their line, cellular = genuinely moving. Real fix for both is
  **WSS on 443 (`webrtcRouteViaSbc`) + TURN over TCP 443**. See
  `AGENT_HANDOFF_FILTERED_INTERNET_2026-08-03.md`.
- **⛔ Never read a raw reconnect count as instability — split it first.** Of 128 reconnects
  on Luxure ext 101 (2026-08-02), **80 were under 5 seconds** (lease renewal, invisible to
  callers) and only **33 were ≥30 s**. 55 sessions sat at a clean **~840 s / 14-minute
  metronome — a fixed interval is a timer, not a network fault**. Real outages arrive in
  *clusters* (proxy); a device physically moving between networks gives isolated single drops.
  Also check for an app update before calling a multi-hour gap an outage — a 3.4 h "outage"
  that day was an upgrade, matched to the second by the `SESSION_START` `appVersion` change.
...
## Onboarding automation (wizard → VoIP.ms → VitalPBX → Connect)

> Full context: `docs/ai-context/AGENT_HANDOFF_ONBOARDING_AUTOMATION.md`.

- **VitalPBX panel deletes are a silent no-op without the confirm step.**
  `{class, method:"delete", mode:"delete", data:id}` returns `state:"success"`
  but only renders a confirmation modal (or a refusal dialog if the object is
  referenced). Actual deletion requires re-POSTing the modal's hidden inputs
  (`mode:"deleteConfirmed"`), then verifying the object is gone by re-listing.
  Two wipes (2026-07-27) "succeeded" while leaving every trunk / outbound
  route / route selection behind — later builds silently ADOPTED the leftovers
  via the case-insensitive idempotency pre-checks. Reference implementation:
  `scripts/onboarding/_wipe-round2.mts`. Delete order: tenants (REST v2, the
  only v2 delete that exists) → ars → trunk_group → trunks; `applyChanges`
  after. REST `deleteTenant` can exceed 20 s — poll `listTenants` for absence
  on timeout instead of assuming failure.
- **Panel error dialogs hide inside `state:"success"` responses.** Always run
  `dialogErrors()` / check for `module-error-list` before trusting a panel
  write (`apps/api/src/onboarding/panelClient.ts`).
- **Fixed 2026-07-27 — VoIP.ms `used_username` permanently bricked a
  submission** (`db4453f8`). An interrupted run created the subaccount during
  a VoIP.ms outage; retries died on `used_username` because the reuse path's
  password rotation sent a partial `setSubAccount` (it's a FULL update),
  failed, was swallowed, and fell through to a doomed create. Now: rotation
  resends full settings; `used_username` re-looks-up and reuses; rotation
  errors surface. Regression tests in `voipMsProvisioning.test.ts`.
- **Fixed 2026-07-27 — number-search cache hid freed spare DIDs** (`b3081f67`).
  The 10-min cache stored the merged payload; spares changed under it. Now
  only the slow purchasable search is cached; spare stock is fetched fresh.
- **VoIP.ms outages return Cloudflare 521/522 HTML** instead of JSON (observed
  ~10 min, 2026-07-27). Scripts must detect a leading `<` and retry with
  backoff; onboarding events show `no_response` for runs caught in one.
- **Invitation emails require globally unique addresses.** One account per
  email across ALL tenants — a stress run reusing an email logs "Email already
  in use by another organization" and correctly sends 0 invites.
- **Fixed 2026-07-27 — wizard autosave 500'd on every step** (numeric
  `currentStep` vs string schema; `z.coerce.string()` now). The resulting 4xx
  storm plus websocket reconnect churn got a tester's IP auto-banned by
  nginx; `173.212.214.198` (Ezra) is now allowlisted in
  `/etc/nginx/connectcomms/allowlist.conf`.
