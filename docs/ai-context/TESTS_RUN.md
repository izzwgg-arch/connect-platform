...
- **The stale sentence was in a TOOL DESCRIPTION, where no knowledge fix could reach it.** Asked "what can you help me with on this computer?" the deployed workspace answered with the 2026-09-02 card world verbatim — the three kinds, the three folders, "each task appears on screen and only runs after you press the button". Verified on the live database that the published knowledge row was already CORRECT (new wording present, card-era wording absent, `updatedAt` four minutes before the turn), and the prompts were correct too. The text was the `description` of `coworker_task` / `my_computer_tasks`, which a model reads whether or not it ever calls the tool, and which only stepped aside when `handsOn` was true — the app was reconnecting, so it was false.
- **Fix + guard:** those two tools step aside on EVERY workspace turn now (`dropProposalEra = handsOn || !!ws`). New test `the card-era proposal tools are never offered on a workspace turn — hands connected or not` — **replayed RED against the previous condition** (`AssertionError: the card tool must not be offered in the workspace`) and green after, and it also asserts the dock is untouched (no turnId ⇒ the proposal tools still offered, so FloatingAssistant's card path is unchanged).
- **Then the honest sentence underneath exposed a fourth defect: the FULL PAGE could never touch the computer.** `/coworker` replied "right now the app isn't connected" on a screen whose own pill read **"On this computer: connected"**. The hands were gated on `viewingPath.startsWith("/desktop/coworker")` — bubble only — so the full page, the one surface where folders and git projects are attached, was locked out of the feature those attachments exist for. Now gated on `isCoworkerPath`. ⛔ No security property widened: `viewingPath` is CLIENT-SUPPLIED, so any caller could always have claimed the bubble's path; the boundary is, and remains, the desktop's own policy core plus its NATIVE approval window, with the hands keyed to `{tenantId, clientUserId}`.
- agent `node --experimental-test-module-mocks --import tsx --test "src/**/*.test.ts"` → **890 tests, 888 pass, 2 fail** — the same two DOCUMENTED pre-existing failures (`corpus/archive`, `transcription/everett`); neither file touched. agent `tsc -p tsconfig.json --noEmit` → **0 errors in any file this work touched** (the `unref` DOM-lib + `packages/db` moduleResolution baseline only).
- The `desktopLink.ts` source guard that pinned `ctx.desktopApp || inBubble` is REWRITTEN rather than deleted: it now pins `ctx.desktopApp || onCoworkerSurface`, pins the single `isCoworkerPath` definition, and **forbids `startsWith(COWORKER_CHAT_PATH)` from coming back**.
- ✅ **DEPLOYED AND CONTAINER-VERIFIED, twice.** Both rebuilds followed the clone-reset rule (`git reset --hard origin/<branch>` in `/opt/connectcomms/app` first — the deploy queue was idle, checked, and the only `pgrep` match was the checking command itself). Verified INSIDE the running container, not from the build log: `dropProposalEra` ×2 and the comment naming the defect ×1 after the first; `onCoworkerSurface` ×3 after the second; `class ActivityHub` still present; `healthy`, restarts **0**.
- ✅✅ **THE THING THAT HAD NEVER BEEN PROVEN IS PROVEN: a real task ran on a real computer, driven from the FULL PAGE.** Asked "Tell me this computer's Windows version and how much free disk space is on C:." on `/coworker` in a browser, the workspace ran it end to end: two live plain-English steps (**This computer** → "Checking this computer" 6s, **Command** → "Running a command on this computer" 2s), header **Done · 2 of 2 steps · 28s**, the RIGHT NOW panel showing the computer with both steps ticked, and THIS TASK counting **Steps done 2 · Time 28s · Things changed None · Asked you 0 times** with MADE OR CHANGED correctly empty for a read-only job. The answer carried REAL values off that machine — `Microsoft Windows Server 2025 Datacenter 10.0.26100 (Build 26100)`, `320.22 GB free of 399.68 GB (80.1% free)`. No tool names, no code, no JSON anywhere on screen. ⛔ This same request was impossible ten minutes earlier: it is the `04fb30b9` gate fix that made the full page able to reach the computer at all.
- ✅ **PROVEN ON THE LIVE SCREEN — the same question, re-asked after the deploy.** The answer is now: *"When the Loopcom Windows app is open and the bubble is on, I can work directly on this PC: find, read, and organize your files, make and read spreadsheets, use a separate Loopcom browser to open sites and download files, work with code projects, and check system details and run diagnostics — always asking before I change anything."* No button press, no three-task list, and it matches the published knowledge document.

## The Coworker workspace: live steps, questions back, voice, files/folders/git (2026-09-15, `7b26c388`)

- agent `node --experimental-test-module-mocks --import tsx --test "src/**/*.test.ts"` → **885 tests, 883 pass, 2 fail** — the two are the DOCUMENTED pre-existing `corpus/archive` and `transcription/everett` failures (neither file touched). 40 of the passes are new: the activity hub (ownership, turn reuse, escapable questions, sweep, event cap), the step labels (a guard that a label can never contain a tool name, JSON, a PowerShell script or a typed value), the two turn tools, file reading (docx/pptx/xlsx/odf/rtf/html/PDF-with-ToUnicode; a scanned PDF, an encrypted PDF and a zip bomb all reported honestly), the workspace turn through the REAL router tool loop (plan + steps + recorded step, ask_person answered from the page, Stop mid-question, the phone switch removing tools, no turnId ⇒ byte-for-byte the old behaviour, task-continuation ownership, attachment text + transcript reaching the model), and the routes through a real Fastify with a real portal JWT.
- agent `tsc -p tsconfig.json --noEmit` → the pre-existing baseline only (`unref` DOM-lib + `packages/db` moduleResolution); **0 in any file this work touched**.
- desktop `node --import tsx --test src/coworker/*.test.ts src/coworkerWidget/*.test.ts` → **67 pass / 0 fail**. New: git against a REAL temp repository — a commit message full of shell and git syntax stays ONE argument and deletes nothing; clone refuses `ext::`/`file://`/local paths; push to an unreachable remote fails in seconds instead of hanging on a hidden credential prompt; a switched-off tool family is refused BEFORE any verdict or approval; webmail is blocked unless Email is switched on; push still asks under Full access; and the bridge's rules (raising access needs the native dialog, another origin is refused on every verb, a dropped folder outside home asks, drive roots and Windows folders are refused).
- ⛔ desktop **proven in a CLEAN export** (`git archive` of the commit + my six shared files, junctioned node_modules): `tsc` **0 errors**, 64/67 tests pass, the 3 failures being only files outside the export (a brand script, the agent's routes, the portal's assistant). This mattered because a second session was editing the same six files at the same time.
- portal `npm test` → **650 tests, 646 pass**; the 4 are pre-existing or another session's (campaigns index layout, `checkOfferCompatibility`, the CoworkerTaskCard string test that already failed at HEAD, the desk-phone provisioning listener). portal `tsc` **0 errors**. shared **70/70**.
- **Stress, local** (`apps/agent/scripts/coworkerWorkspaceStress.ts`, committed): 400 turns × 40 steps = 48,800 events written in 436 ms, read in 15 ms, heap 9→26 MB; 200 rounds proving a POLLED view (batched, duplicated, reversed, stale re-reads) equals the whole-stream view; question races (a second answer refused, skip/stop/timeout each resolving exactly once, a question after Stop refused rather than hung); sweep reclaims every finished turn at its TTL; **2,000 fuzz turns** — no throw, nothing left "running" after done, no question surviving the end.
- **Stress, live against the deployed agent** (`/root/coworker-live-stress.sh`, synthetic identity, no customer data): 1,000 polls across 40 turns → p50 **70 ms**, p95 **220 ms**, max 394 ms; 20 concurrent settings saves all 200; unauthenticated **403**, malformed turn id **400**, unknown turn **404**, malformed JSON **400**, oversized answer **400**; the agent healthy afterwards, 0 restarts, 0 error lines.
- ✅ **DEPLOYED AND CONTAINER-VERIFIED.** agent rebuilt from `7b26c388`: `ActivityHub`, `ask_person`, `show_plan`, `registerCoworkerUiRoutes`, `transcribeForCoworker`, `extractText`, `STOPPED_REPLY`, `coworkerWorkspacePrompt` all grepped INSIDE the running container; all eight new routes answer **403** without a token through nginx while an unknown route **404**s. portal deployed (branch tip): `/coworker` and `/desktop/coworker` **200 on both hostnames**, and `.next` carries the page, `cw-steps`, “Coworker needs your OK” and `can_view_workspace_coworker`.
- ✅ **PROVEN ON A REAL SCREEN (the one thing tests cannot show):** in Izzy's own Chrome, signed in, `/coworker` rendered the workspace; a typed task ran live (steps block → “Thinking…” → a plain-English reply in 13 s), the task took its own URL and appeared in the task list, and the right-hand panel counted steps/time/changes. ⛔ Two real defects came out of that one minute: the composer kept the sent text until the whole TURN ended (it clears immediately now — `send` only resolves when the task is finished, which with the hands is minutes), and the stored knowledge document still describes the CARD-era “press the button” tasks (recorded as a follow-up; not a regression of this work).
- ⏳ **NOT PROVEN:** nobody has run a task with the HANDS connected (no file moved on a real computer), nobody has used voice, drag-and-drop, folder/repo attach or the access dialog, and desktop **rc.17 is built but NOT installed and NOT published**.
...
## 2026-09-15 — Desk phones, round 18: the customer's own scan link

- `apps/api` `node --experimental-test-module-mocks --import tsx --test src/deskPhoneSetup/*.test.ts` — **287 tests, 286 pass, 0 fail, 1 skipped** (was 278; **+9 new** scan-link tests). Covers: the customer projection (and that the serial never crosses it, only `serialOnFile`); match-by-MAC with the maker named from the OUI; an address not on the order refused **409 naming the maker** (a Yealink sticker against a Grandstream order — the brand-agnostic case); the typed fallback through the same gate; missing/revoked/expired as ONE flat refusal that never says which; one-live-link-per-run on re-mint; run isolation; another company refused; and the permission gate on minting.
- `apps/api` `node --experimental-test-module-mocks --import tsx --test src/publicReadyJwtBypass.test.ts` — **12 pass, 0 fail**, including the new anchoring test: `/phone-setup/<token>` and `/api/phone-setup/<token>` open; `/desk-phones/runs/:id/scan-link` and `.../revoke` stay gated; `/x/phone-setup/…` and `/admin/phone-setup/tokens` do NOT inherit the bypass.
- API `tsc --noEmit -p tsconfig.json` — **no errors in any file touched this round**; the ~20 reported diagnostics are the documented pre-existing ones in `server.ts`, `webrtc*`, `supermarket/` and `packages/db`, none of them mine. Portal `tsc` — **0 errors**.
- ⛔ **A harness fix that mattered:** the fake table gained `findUnique` (the scan link is looked up by its unique token hash — production uses it, so a fake without it would have been the only thing failing) and `applyAtomics`, so a Prisma `{ increment: 1 }` really increments instead of writing the object into the column and making any assertion on `scanCount` meaningless.
- ⛔ **Two failures on the first run were the TEST being wrong, not the code — both worth recording.** (1) `formatMac` UPPER-cases; I had trusted its doc comment (`80:5e:0c:4d:7e:6b`) instead of its behaviour. (2) `POST /desk-phones/runs` **RESUMES** the tenant's live run ("one live run per customer") rather than creating a second, so two `runWithPhone` calls are ONE run — the isolation test read `total: 2` and was correct to; it now closes the first run so a second genuinely exists. A test that had "passed" without that would have proven nothing.
- ⛔ **A stale-worktree hazard was caught before committing:** `AGENT_HANDOFF_DEVICE_IDENTIFICATION_2026-09-14.md` was dirty in the DELETION direction (24 lines, another session's "§10p Round 17 — retCode 0 ≠ delivered" record). The worktree copy was OLDER than HEAD, so committing it would have destroyed a real lesson. Backed up to the scratchpad and restored from HEAD **before** appending §10q.
- ✅ **DEPLOYED AND CONTAINER-VERIFIED `7d93d23a` (2026-09-15/16).** **api**: `69ba6ae4` → `7d93d23a806c…`, blue/green cutover clean, restarts **0**, health **200**; my code read from the **running** container at `/app/apps/api/src/…` (`phone_not_in_order` ×1, `isPublicPhoneSetupPath` ×2, `deskPhoneScanToken` ×7) — ⛔ grep `src/`, never `dist/`, which does not exist in this image. **portal**: `6e3cc2b5` → `7d93d23a806c…`, restarts **0**, health **200**; the shipped build carries `.next/server/app/phone-setup/[token]/page.js`, the client chunk `page-626553e8ad77d08e.js`, the layout chunk, and `ps-shell` in the CSS bundle.
