...
- **Voicemail drops:** list/use/drop/upload/edit/archive routes use `requireCrmAccess` (Agent + Manager + CRM Admin). PBX/system recording settings remain outside this feature (`/pbx/call-recordings`, admin-only). **Playback caveat (2026-06-09):** the `POST /crm/voicemail-drops/drop` *playback* step is currently **non-functional on production** — it relies on ARI `/channels/{id}/play`, which needs a Stasis app the PBX build does not provide (no `res_ari_websockets.so`; `GET /ari/applications` → `[]`). Upload/list/permissions work; the on-call audio playback does not. The sustainable fix is AMI `Redirect` into a `[connect-vm-drop]` dialplan context. Full architecture, root cause, and limitations: `TELEPHONY.md` § Voicemail Drop (CRM) and `docs/pbx/connect-voicemail-drop-context.conf`.
...
| Campaigns — edit | `/crm/campaigns` row menu + detail | `PATCH /crm/campaigns/:id` | `requireCrmAccess` + `assertCrmCampaignAllowed` | Yes (assigned) | Yes (tenant) | Yes (tenant) | Yes |
| Campaigns — delete | same (UI label **Delete**) | `DELETE /crm/campaigns/:id` → `status: ARCHIVED` | `requireCrmAccess` + campaign scope | Yes (assigned) | Yes (tenant) | Yes (tenant) | Yes |
| Funders — edit | `/crm/funders` row menu + detail | `PATCH /crm/funders/:id` | `requireCrmAccess` + `tenantId` | Yes | Yes | Yes | Yes |
| Funders — delete | same (UI label **Delete**) | `DELETE /crm/funders/:id` → soft-delete | `requireCrmAccess` | Yes | Yes | Yes | Yes |
...
| Contacts — delete | same (UI label **Delete**) | `DELETE /crm/contacts/:id` → soft-delete | `requireCrmAccess` + contact scope | Yes (in scope) | Yes (tenant) | Yes (tenant) | Yes |
| Contacts — restore | archived detail | `POST /crm/contacts/:id/restore` | `requireCrmAccess` + contact scope | Yes (in scope) | Yes (tenant) | Yes (tenant) | Yes |
...
## Edit / delete behavior (campaigns, funders, contacts)

All three entities use **soft delete** on the API. The portal labels the action **Delete** (not hard-delete of history).

| Entity | Edit API | Delete API | Default list behavior |
|--------|----------|------------|----------------------|
| Campaign | `PATCH /crm/campaigns/:id` | `DELETE` → `status: ARCHIVED` | Excludes archived unless filtered |
| Funder | `PATCH /crm/funders/:id` | `DELETE` → `active: false`, `archivedAt` | Active funders only |
| Contact | `PATCH /crm/contacts/:id` (scoped) | `DELETE` → soft-delete | Active contacts; admins can list archived |

**Portal:** `CrmRowActionMenu` (Edit + Delete), `CrmConfirmModal`, `EditCampaignModal`. Menus visible to anyone with the page nav permission (`can_view_crm_campaigns` / `funders` / `contacts`). List **Edit** opens detail with `?edit=1`.

**Active campaigns:** delete shows a warning that queue work stops; data is retained.
