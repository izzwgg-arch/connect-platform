...
⛔ **"Trimpro runs filtered internet" is unsupported.** `whois 69.118.75.72` →
**Optimum Online**, Hicksville NY, ordinary ISP — not a filtering proxy. And Trimpro is
**not** on the 443 route (`webrtcRouteViaSbc: false`), so the contact IP is genuinely the
customer's own address and the whois test is valid here.
