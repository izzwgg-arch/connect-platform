...
- api `707820cb` — instant-originate: when the requesting client's validated `callerSipEndpoint` is already Avail, skip the wake push + 12s registration wait (the wake churned the shared AOR mid-ring, which is what made Answer spin forever); wake fan-out bounded to devices seen ≤30 days; portal debug readout hidden behind `localStorage.ecpVmDebug`.
- helper v2026.08.04.2 (`1f216a80`) — dispatch dialplan dials CONTACTS not endpoints (`PJSIP_DIAL_CONTACTS(base)` + `(base_1)` expanded at dial time); endpoint-level Dial created one channel per endpoint so only one device of the AOR ever rang. AstDB `connect_vm_dial` demoted to fallback. Rollback backups in `/root` on the PBX.
- api `b6034b7b` — ⛔ the Android ring screen is push-driven; the synthetic `INCOMING_CALL` push is mandatory on every vm-record path (a bare SIP INVITE renders no UI). Only the WAKE push is skipped.
