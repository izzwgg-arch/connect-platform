...
**Connection refused** from the Connect app host (while PBX loopback `/health` works) = helper bind / firewall — see **`DEPLOYMENT.md`** § **listen bind** (env **`CONNECT_PBX_HELPER_BIND`**, not Python edits).
