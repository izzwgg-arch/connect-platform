...
4. **PBX / helper CPU (2026-05):** dense **`POST /voicemail/spool/list`** bursts on **`connect-pbx-helper`** during **`app-worker-1`** scheduled **`voicemail-spool-reconcile-summary`** cycles (full-fleet enumeration). Mitigation: adaptive **`setTimeout`** scheduler + optional **probe skip** + **`sinceOrigtime`** narrowing + env throttles ? see **`VOICEMAIL_FLEET_STALE_RISK.md`** ß5 and **`TELEPHONY.md`** ßVoicemail. Verify with **`journalctl -u connect-pbx-helper`** and worker JSON (**`helper_calls`**, **`skipped_probe_no_new`**, **`next_reconcile_delay_ms`**).
...
Reference: **`docs/ai-context/BILLING.md`** ß SOLA / Cardknox and ß SOLA setup (operators).

---

## Cardknox iFields (admin billing)

1. **Blank card fields:** Confirm **`GET /admin/billing/platform/tenants/:tenantId/sola/public-config`** returns **`ifieldsKey`** (not null). Admin flows use **`@cardknox/react-ifields`** (`CardknoxIFieldsForm` / `PaymentMethodsModal` / `OneTimeChargeDrawer`) ? do **not** reintroduce CDN **`ifield.htm`** iframes without `setAccount` timing fixes.
2. **One-time charge confirm fails:** Tokenization runs on **Review** (before confirm). If the operator changes charge mode away from ?new card? and back, they must review again so a fresh SUT is captured.
3. **PCI:** Browser sends only **`xSut`** to **`POST .../one-time-charges`** or **`POST .../payment-methods/sola/save`**. Grep API logs/events for raw SUT/PAN ? must not appear in **`BillingEventLog.metadata`**.
...
- **Voicemail Call-to-Record (`POST /voicemail/greeting/record-call`):**
  ...
  - **vm-record mobile answer disconnect (Phase E, 2026-05-08 fix).**
    ...
    for `vmr-*` ACCEPT requests. See `KNOWN_ISSUES.md` ß "Phase E".
...
- **Android release APK: contact import verification without Metro (2026-05-12).**
  ...
  **Import from phone** through **Import(N)**. Pull logs, for example:
  ...
  `DEPLOYMENT.md` ß Mobile builds). After an API change to `/mobile/android/latest`,
...
5. For **voicemail ingestion** (new messages missing from web/mobile but calls work):
   ...
   2. **Fast path** ? `docker logs app-telephony-1 ... | grep MessageWaiting` and
      ...
      **Do not** patch helper Python on the PBX ? re-run the **pinned**
      ...
      and confirm **`/health`** flips to **`2026.05.08.1`** (`DEPLOYMENT.md` ß installer only).
      ...
      **`ss -lntp | grep 8757`** ? expect **`0.0.0.0:8757`** before blaming ?Connect is wrong.?
      ...
      (`DEPLOYMENT.md` ß **app-host smoke**). Compare **`sha256sum`** of **api** vs **worker** env to confirm
      ...
      **`DEPLOYMENT.md`** ß **helper secret alignment only** (avoid blind dual rotation).
      If operators **believe** PBX was updated but **401** persists, check **duplicate env keys**, **quotes**,
      **CRLF/trailing spaces**, and that **systemd** loads the file you edited (`DEPLOYMENT.md` ß **Troubleshooting: still 401**).
      ...
      **Fingerprint without pasting secrets:** app-host **`docker exec`** length + **`sha256sum`**, PBX **`/etc/connect-pbx-helper.env`**
      + **`/proc/<pid>/environ`** same ? **`DEPLOYMENT.md`** ß **Secret mismatch fingerprints**.
   ...
   4. **Which PBX runs the helper?** Compare **`PBX_ROUTE_HELPER_BASE_URL`** (api/worker env) to the
      ...
      helper on the host Connect actually calls (`DEPLOYMENT.md` ß Phase 1 verification A?).
   ...
   9a. **Fleet mismatch audit + idempotent backfill (worker container on app host).** Fair scheduling
      ...
      **`docker exec app-worker-1 ?`** on the Connect app host ? **not** on the PBX. Exact CLI,
      ...
      cases are in **`DEPLOYMENT.md`** ß **Voicemail ? operational recovery (audit + backfill)**.
   ...
   9b?. **Scheduled spool reconcile (insert-only, durable pipeline).** Complements the **60s**
      ...
      **`helper_version_ok_global`**, **`stale_high_risk_increased`**, **`top_risky_mailboxes`**. Last run
      ...
      (default **`2026.05.10.1`**), **`VOICEMAIL_HELPER_HEALTH_TIMEOUT_MS`**. Details: **`VOICEMAIL_FLEET_STALE_RISK.md`** ß5.
   ...
   11. **Playback** ? for `src_unsupported` / cannot play: hit
      ...
      **`tenantId`**, **`extension`**, **`folder`** (`INBOX` \| `Old` \| `Urgent`), **`msgNum`**
      (`msg[0-9]+`) ? **200** raw audio (`TELEPHONY.md`, **`DEPLOYMENT.md`** ß Phase 2).
      ...
      **Operator install:** exact **`curl` + `bash`** for **`209.145.60.79`** is in **`DEPLOYMENT.md`** ß **Phase 2 ? operator handoff**.
...
- For production checks, use `scripts/safe-prod-check.ts` to print SET/MISSING flags, HTTP status codes, and simple counts √¢‚Ç¨‚Äù never actual secret values.
