...
**Queue pin / `commit_already_deployed`:** After a successful **real** (`dryRun: false`) **`api`** job, the deploy queue may short-circuit a later enqueue with the same **`commitHash`** (`AGENTS.md`). The check uses the **most recent non–dry-run success** for that service and compares its recorded **`deployed_commit`** to the requested pin; **dry-run successes never count** (so a dry-run cannot block a live rollout). If `/ready`, worker env, and nginx are correct but **`POST /ops/deploy/enqueue`** returns **`commit_already_deployed`** for a SHA you know is not what is running, treat it as a queue/DB inconsistency or a stale pin — confirm **`deployed_commit`** in recent **real** jobs, then follow **`KNOWN_ISSUES.md`** / **`AGENTS.md`** post-deploy SHA checks.

**Operational note:** If an older deploy-queue build incorrectly treated **`dryRun: true`** as deployed, upgrade **`connect-deploy-queue`** to **1.1.1+** (see **`ops/deploy-queue`**) so **`commit_already_deployed`** only reflects **real** jobs.
