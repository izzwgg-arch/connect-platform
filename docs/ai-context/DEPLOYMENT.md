...
**Operations dashboard:** `/admin/storage-health` (Phases 1–5) displays inventory, forensics, and **staged cleanup controls** when `STORAGE_CLEANUP_ENABLED=1`. API container mounts host paths read-only except preflight/precleanup snapshot directory (`/opt/connectcomms/backups/storage-preflight` → `/var/lib/connect/storage-preflight`, JSON writes only). Set `STORAGE_CLEANUP_ENABLED=1` only during approved maintenance windows; revert to `0` after cleanup.
...
**Queue pin / `commit_already_deployed`:** After a successful **real** (`dryRun: false`) job for a target, the deploy queue may short-circuit a later enqueue with the same **`commitHash`**. The check uses the **most recent non–dry-run success** for that **same `service`** and compares the requested pin to **`deployed_commit`**, or **`commit_hash`** when `deployed_commit` was never recorded (**`connect-deploy-queue` 1.1.2+**). **Dry-run successes never count** (1.1.1+). **`deploy_common_git_sync`** must run in the caller shell so stage updates do not drop `deployedCommit` from the state file (1.1.2 deploy scripts). If **`POST /ops/deploy/enqueue`** returns **`commit_already_deployed`** for a SHA you believe is not running, treat it as inconsistency — confirm recent **real** job rows and **`AGENTS.md`** post-deploy SHA checks.

**Operational note:** Upgrade **`connect-deploy-queue`** to **1.1.2+** for reliable **`commit_already_deployed`** after real pins (and **1.1.1+** minimum so dry-runs never satisfy the skip).
...
- **`origin` is an HTTPS remote (changed 2026-08-13) — do not switch it back to SSH without a working key.** The clone's **`origin`** was **`git@github.com:izzwgg-arch/connect-platform.git`**; GitHub began rejecting that deploy key with **`Permission denied (publickey)`**, which fails the queue's **`git-sync`** stage for **every** service — the job ends **`status: failed`**, **`currentStage: git-sync`**, **`deployed_commit: null`**, and containers are never touched (no partial rollout). **`origin`** now points at **`https://github.com/izzwgg-arch/connect-platform.git`**, the same URL as the pre-existing **`izzwgg`** remote whose stored credentials still authenticate. Recognise it by **`git fetch origin --prune failed`** in **`/var/log/connect-deploys/<job-id>.log`**. If a new SSH deploy key is installed, revert with **`git remote set-url origin git@github.com:izzwgg-arch/connect-platform.git`** and re-test **`git fetch origin --prune`** *before* enqueueing.
- **Confirm the live deploy branch before copying the `--branch main` examples above.** As of **2026-08-13** production **`api`**/**`portal`** jobs run from **`feat/ivr-migration-takeover`** (which contains **`origin/main`**), and the host clone is checked out on it. Read it from the host — **`cd /opt/connectcomms/app && git branch --show-current`** plus the last successful job per service in **`GET /ops/deploy/jobs`** — never from this doc's examples.
