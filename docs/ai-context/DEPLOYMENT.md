...
**Operations dashboard:** `/admin/storage-health` (Phase 1.5 UI, Phase 1.6 host visibility, **Phase 2 Operations Center**) displays inventory, dependency graph, rollback coverage, confidence distribution, cleanup readiness, BuildKit/APK/log analysis, and preflight snapshot status — read-only, no execution. API container mounts host paths read-only (`docker-compose.app.yml` § `api` volumes) except the dedicated preflight snapshot directory (`/opt/connectcomms/backups/storage-preflight` → `/var/lib/connect/storage-preflight`, JSON writes only). Docker inventory via whitelisted `GET` paths on `docker.sock` (`/system/df`, `/images/json`, `/images/{id}/json`, `/containers/json`, `/volumes`).
