...
## 26. DESIGN (2026-09-15, not yet implemented): audit.jsonl rotation — the helper's audit trail is 84.8 GB and grows unbounded

**The fact (read-only observation, 2026-09-15):** `/var/lib/connect-pbx-helper/audit.jsonl`
= **84,808,228,474 bytes (~85 GB)**. Disk 63% used, 173 GB free — not urgent, but unbounded.
This section is the DESIGN for fixing it as a helper change; installs stay Izzy's
Run button, never a direct write to the READ-ONLY box.

### Why it grew: the audit logs FULL RESULTS, and one action's results are megabytes

`audit()` (helper, ~line 226 on the `526b56ba` lineage) appends one JSON line per POST —
`payload` (bytesB64 stripped, secrets redacted) **plus the complete `result`**.
`/voicemail/spool/list` goes through the same generic dispatch, so **every poll writes the
entire mailbox listing into the audit file** — Gesheft 101 alone holds ~9,100 messages
(≈1–2 MB of JSON per listing), and the api/worker polls lists continuously (the 2026-08-12
flood peaked at ~126 req/min). That dominance is inferred from the code, not yet measured
on the box — the installer step below prints a sampled per-action byte breakdown to
confirm it at install time.

### The two facts that make the fix easy (both verified in source)

1. **`audit()` opens the file per append** (`CFG.audit_file.open("a")`, close after) — no
   long-lived handle. So a **rename rotation needs no service restart, no signal, no
   copytruncate**: the next audit line simply recreates the file.
2. **The unit already grants exactly the needed write**: `User=asterisk`,
   `ProtectSystem=strict`, and `/var/lib/connect-pbx-helper` is in `ReadWritePaths` (§17).
   The helper can rotate its own file **in-process** — no root, no sudo (impossible anyway
   under `NoNewPrivileges=true`), no new units. §17a is the standing lesson on root-side
   moving parts; this design adds none.

### Change A — cap the `result` written per entry (this is what stops the growth)

- New knob `CONNECT_PBX_HELPER_AUDIT_RESULT_MAX_BYTES` (default **8192**), read in `Config`.
- In `audit()`: after redaction, serialize `result`; if it exceeds the cap, replace it with
  a summary — `{"auditTruncated": true, "resultBytes": <n>, "resultKeys": [...top-level
  keys...]}` (for a list result, `"listLen"` instead of keys). `action`/`ok`/`error`/
  `payload` stay intact; apply the same cap to `payload` as a backstop (payloads are small).
- Real writes (retarget, mirror, console) have small results and keep auditing in full;
  only bulk reads get summarized. A spool/list line drops from ~1–2 MB to ~300 B.
- ⛔ Do NOT stop auditing successful reads instead — the audit line RATE was the evidence
  that proved the 2026-08-12 flood fix (3,074/hr → 0). Keep one small line per request.

### Change B — size-based rotation in-process (this bounds the file forever)

- Knobs: `CONNECT_PBX_HELPER_AUDIT_MAX_BYTES` (default **268435456** = 256 MB) and
  `CONNECT_PBX_HELPER_AUDIT_KEEP` (default **4** rotated files → worst case ~1.25 GB).
- Module-level `threading.Lock`; `audit()` takes it around stat + rotate + append (requests
  are capped at 32 in-flight; the serialization cost is nil, and it makes rotation strictly
  race-free against the per-call opens).
- When `st_size >= max`: shift with `os.replace` (atomic, same fs): unlink `.KEEP`, then
  `.3→.4`, `.2→.3`, `.1→.2`, base→`.1`; then append as before.
- Everything stays inside the existing best-effort `try/except OSError` — the invariant in
  the current comment holds: **audit/rotation failure must never fail the response** (a
  completed write reported as failed makes the client retry a write that already happened —
  the exactly-once trap).
- No gzip in the request path (CPU spike on a live PBX for nothing — plain text at these
  caps is fine). journald is volatile on this box (§17a), so don't lean on the journal for
  rotation-failure evidence either; the stderr line is best-effort only.

### Change C — the one-time 85 GB backlog (installer step, root, Run button)

- Before the service restart: if `audit.jsonl` is > 2× `AUDIT_MAX_BYTES`, `mv` it to
  `audit.jsonl.oversize-<YYYYMMDD>` — a same-filesystem rename, instant, safe (per-append
  open). Print the archive's size.
- Print a **sampled** per-action count/byte breakdown from the archive's head (e.g. first
  100 MB — never scan 85 GB inside the Run button) to confirm the spool/list theory.
- ⛔ **The archive is NOT deleted by default.** 85 GB of audit history is Izzy's call, not
  the installer's: delete only when run with `CONNECT_PURGE_OLD_AUDIT=1`, otherwise the
  installer prints the one-line `rm` for later. 173 GB free means no urgency.

### Rejected alternatives

- **logrotate / cron / systemd timer (root):** mechanically fine (per-append open means
  plain rename rotation works), but it adds a root-owned moving part outside the helper's
  tested surface (§17a), splits file ownership between an asterisk writer and a root
  rotator, and needs nothing the in-process version doesn't already have.
- **copytruncate:** solves a long-lived-fd problem the helper doesn't have, and loses lines.
- **Helper truncates the backlog itself at startup:** startup shouldn't spend minutes of
  I/O, and deleting history is not the code's decision.

### Implementation gotchas (all documented traps, restated so nobody rediscovers them)

1. ⛔ **The helper source exists TWICE**: `scripts/pbx/vitalpbx-inbound-route-helper.py`
   AND embedded as the `PYHELPER` heredoc inside
   `scripts/pbx/install-vitalpbx-inbound-route-helper.sh`. Change BOTH, byte-identical.
2. ⛔ **Build on the branch tip that carries `526b56ba`** (today: `feat/ivr-migration-takeover`;
   helper 4,618 lines). `main` and the older `claude/*` branches carry a stale 863-line
   pre-mirror helper — building there is the `1b0771bb` 13-commits-behind merge trap again.
3. ⛔ Before install, `sha256sum` the live `/opt/connect-pbx-helper/vitalpbx-inbound-route-helper.py`
   against the base the change was built on — a mismatch means silent downgrade (voicemail
   flood handoff's rule).
4. Bump `VERSION` (e.g. `2026.09.15.1`); add installer-suite source guards for the lock,
   the rotation, and the result cap — ⛔ asserting on **executable lines only**, never on
   doc comments (the guard-test comment trap, hit three times, §17 tail).
5. The knobs default in code — no `/etc/connect-pbx-helper.env` change needed; the
   EnvironmentFile can override them later if wanted.

### Sizing math (why these defaults)

Current growth ≈ 1.4 GB/day (85 GB since ~mid-July). With the result cap, even the
2026-08-12 worst case (126 req/min of spool traffic) writes ≤ ~55 MB/day, so the
5 × 256 MB window holds 3+ weeks of worst-case history and months of normal — versus
under ONE day of history if rotation shipped without the result cap (rotation alone is
not the fix; the result cap is).
