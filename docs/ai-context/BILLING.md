...
- **Tenant context (2026-05-17):** Admin Billing uses the **global workspace switcher** only — no in-billing company rail. **Tenant scope** → all sections scoped via `?tenantId=` (synced from `useAppContext`). **All workspaces** → cross-tenant overview and list tabs; tenant-scoped ops (methods, activity, charge customer) require selecting a workspace in the header or a deep link with `?tenantId=`.
