...
- **CRM dashboard hierarchy (Phase 19B)** — `/crm/dashboard` is the only route that uses dashboard chart primitives (`components/crm/charts/*`, `components/crm/dashboard/*`). Visuals must map to **real** API fields (contacts stats, daily report, follow-ups, campaigns, queue preview, tasks stats, import batches). No fabricated time series, sparklines without backing data, or marketing widgets. Prefer charts + compact KPI strips over paragraph hints and equal-weight text cards.
  - 19B.3: One metric = one primary home. Do not duplicate the same stat across multiple widgets unless the second placement adds different action context (e.g., KPI row count vs. actionable link in Action Required).
  - Queue Pressure gauge is the preferred centerpiece in Live CRM Operations.
  - Donut charts only for real segmented data (3+ meaningful segments). Avoid donuts for 2-part splits.
  - Micro charts are allowed only when based on real data or as clearly neutral current‑load placeholders derived from the current value (not historical trends).
  - All visuals must remain theme‑aware in both dark (navy/glass) and light (white/slate) portal themes.
- **CRM campaign operational workspace (Phase 19E+ / 19E.6)** — `/crm/campaigns` and `/crm/campaigns/[id]` are outbound **command centers** using **`mk.workspace`** (`.crm-campaign-cinema`) + `campaignCinemaClasses.ts` / `globals.css` cinema hooks — **not** legacy `crm.campaignWorkspace` forced-dark island. **Dual theme (19E.6):** polished **light** cinema (frosted SaaS surfaces, pale cyan gradients, readable dark text) when portal `data-theme=light`; preserve **dark** neon cinema mockup when `data-theme=dark` — no black island in light mode, no light leak in dark mode. Use scoped `--cinema-*` / `--crm-*` and hook classes; ban hardcoded dark hex in campaign components. **No duplicate metrics**: `CampaignCommandHeader` owns counts; `CampaignPerformancePanel` is funnel/status mix only. Detail stack: header → performance → `CampaignDetailCommandPanel` (ops grid) → members roster. **Index (19E.2–19E.3):** Volume/Outcome clusters; queue pressure from real report fields only; tiered Open/Queue/Power/Pause/Archive; `mk.filterPill*` on sticky bar. Never invent metrics; preserve queue/import/distribute behavior.
...
- **CRM contacts index (Phase 19D.1 / 19D.2 / 19D.3)** — `/crm/contacts` uses `crm.pageInnerContacts` + **`crm.contactsWorkspace`** (`.crm-contacts-workspace` in `globals.css`) + `CRMPageHeader` + visual `SummaryStatTile` strip. **Not a database directory UI** — wide desk layout, command search bar, divided relationship rows with prominent Open/Workspace actions. **Dual theme (19D.3):** must render polished **light** surfaces when portal `data-theme=light` (no black-card island) and preserve **dark** 19D.1/19D.2 premium styling when `data-theme=dark` — do **not** force campaign-style dark-only token overrides in light mode. Command bar: stage/list/assigned pills = `crm.filterPill*`; search = `crm.input`; checkboxes = `crm.checkbox`; bulk select = `crm.selectCompact`. Page-scoped stats only (no fake insights). Preserve search, pagination, stage/archive/assigned filters, bulk assign, import link, permissions.
...
- **CRM live call workspace (Phase 19F)** — `/crm/live-call` uses `components/crm/live/*` + `crm.pageInnerLive`. **No fake live call state** — telephony badge/timer only from `useTelephony()` when a matching call exists; otherwise show explicit idle/context copy. Idle route must be a **ready desk** (`LiveWorkspaceIdle`), not a centered “no contact” dead end. All panels use real APIs (contact, timeline, tasks, scripts, checklists, disposition, SMS, originate, queue PATCH). **Workspace continuity:** preserve screen-pop and queue URLs (`contactId`, `memberId`, `campaignId`, `returnTo`, `linkedId`, `from`, `mode=power`); do not rewrite routing or PBX/call-control backends.
  - 19F.2 visual posture: Large contact hero; distinct live status states (idle/dialing/connected/ended), compact session rail, scripts/checklists/tasks action rail.
  - Post-call wrap-up: disposition buttons are large and keyboard-addressable; numeric hints visible. `Enter` saves; `Shift+Enter` saves + advances when `mode=power`. Use progressive disclosure for follow-up (e.g., show quick-time chips only on Callback).
  - Sticky wrap-up bar is required: Save and Save + Next (power) with keyboard hints. Do not block content; keep premium, compact density.
  - Dark/light: All visuals use CRM tokens for both themes. No ad-hoc grayscale backgrounds or forced dark islands. Maintain readable contrast and subtle shadows.
...
93. **`/crm/scripts` must use the scripts playbook visual system (Phase 19I / 19I.1 / 19I.2).**
    - Layout: `ScriptCommandHeader` + 3-column grid (`scriptsGrid` side rails + flexible center) + `ScriptQuickTipsStrip`. Shell: `CRMPageShell innerClassName` with `pageInnerScripts` + `scriptsWorkspace`.
    - **Dual theme (19I.2):** `.crm-scripts-workspace` must render polished **light** surfaces when portal `data-theme=light` and preserve **dark** 19I.1 premium styling when `data-theme=dark`. Do **not** force a dark-only token island in light mode. Do **not** regress dark glow/depth.
    - Premium surfaces: theme hooks in `globals.css` + `crm.scripts*` tokens — accent template cards via `SCRIPT_TEMPLATE_ACCENT_CLASSES`; no flat gray template slabs.
    - Idle center uses `ScriptWorkspaceIdle` (onboarding + CTAs), not `CRMEmptyState` alone.
    - No raw `bg-white` / `bg-gray-50` utilities on script routes; use `crm-*` semantic colors + workspace CSS.
    - `ScriptEditModal` uses `crm.scriptsEditModal` + `serializeScriptSections()` — no forced `[color-scheme:dark]` on the modal card.
