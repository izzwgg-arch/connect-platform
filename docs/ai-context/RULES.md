...
- **CRM queue workbench (Phase 19C)** — `/crm/queue` uses `crm.pageInnerQueue` (wide, ~1440px) and a **12-column** feed + overview + attention layout. No `max-w-3xl` centered column or large dead side margins. Operational side panels and empty states must use **existing** APIs (`/crm/queue`, `/crm/tasks/stats`, `/crm/campaigns`) — no fake “AI recommendations”, fabricated progress, or placeholder metrics. Power mode is a visible **session bar** (`QueuePowerSessionBar`), not a small mode chip.
