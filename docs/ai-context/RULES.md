...
- **CRM campaign operational workspace (Phase 19E)** — `/crm/campaigns` and `/crm/campaigns/[id]` are outbound **command centers**, not metadata CRUD. Use `components/crm/campaign/*` + Phase 19A tokens. Index cards must show workload signals from `GET /crm/reports/campaigns` (queue work, callbacks, conversion %) when available — never invent metrics. Detail charts derive from `statusCounts`, `memberCount`, and `GET …/workload` only. **No** fake AI analytics, live polling, or demo conversion curves. Members are operational cards (`CampaignMemberCard`), not spreadsheet rows. Imports are event cards in the sidebar, not buried one-liners. Queue links (`campaignId`, `mode=power`) are primary navigation — do not hide them in settings collapses.
