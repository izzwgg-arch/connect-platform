...
- **CRM campaign operational workspace (Phase 19E / 19E.1)** — `/crm/campaigns` and `/crm/campaigns/[id]` are outbound **command centers**, not metadata CRUD. Use `components/crm/campaign/*` + Phase 19A tokens. **No oversized sparse cards** — index `CampaignIndexCard` is compact and horizontal; use `crm.pageInnerCampaign` wide layout. **No light-token leakage** on campaign surfaces: ban `bg-white`, `bg-gray-50`, `bg-green-100`, `bg-amber-50`, `text-green-700`, light modal textarea/priority pills, blue bulk bars — use `crm.input`, `crm.campaignPriorityPill*`, `crm.campaignModalBackdrop`. Empty states use `CampaignGuidedEmpty` (numbered steps), not giant dashed `CRMEmptyState` slabs. Index cards must show workload signals from `GET /crm/reports/campaigns` when available — never invent metrics. Detail charts derive from `statusCounts` and `GET …/workload` only. Members are `CampaignMemberCard` rows. Queue links (`campaignId`, `mode=power`) stay primary in header/card actions.
- **CRM queue workbench (Phase 19C / 19C.1)** — `/crm/queue` uses `crm.pageInnerQueue` (wide, up to ~1680px) and a **12-column** feed + overview + attention layout (feed widens on `2xl`). No `max-w-3xl` centered column or large dead side margins. **No light-token leakage** on queue/power surfaces: ban `bg-white`, `bg-gray-50`, `bg-green-50`, `bg-blue-100`, and similar on `/crm/queue` — use `crm-*` tokens (`queueCountPill`, `bannerSuccess`, `btnCallSuccess`, status chips from `queueUtils`). Operational side panels and empty states must use **existing** APIs only — no fake recommendations. Power mode uses `QueuePowerSessionBar` with dark chips throughout.
- **CRM contacts index (Phase 19D.1)** — `/crm/contacts` uses `crm.pageInnerContacts` + `CRMPageHeader` + visual `SummaryStatTile` strip. **Not a database directory UI** — wide desk layout, command search bar, divided relationship rows with prominent Open/Workspace actions. Page-scoped stats only (no fake insights). Preserve search, pagination, stage/archive/assigned filters, bulk assign, import link, permissions.
