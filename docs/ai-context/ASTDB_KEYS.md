...
|---|---|---|---|---|
| `connect_vm_dial` | `T<tenant>_<ext>` | string | Endpoint-level PJSIP dial string (e.g. `PJSIP/T21_101&PJSIP/T21_101_1`) — since helper v2026.08.04.2 this is a **fallback only**: the dispatch dialplan first expands `PJSIP_DIAL_CONTACTS(base)` + `PJSIP_DIAL_CONTACTS(base_1)` at dial time so every registered contact rings (endpoint-level Dial creates one channel per endpoint, not per contact) | `vm_record_call()` in PBX helper before originate |
