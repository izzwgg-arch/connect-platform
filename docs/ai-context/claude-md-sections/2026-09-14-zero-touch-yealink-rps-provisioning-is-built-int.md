...
- ✅ **2026-09-15 (later still) — THE WIZARD'S PASSWORD DEAD-END OFFERS THE ZERO-TOUCH PATH (`7931e768`, portal DEPLOYED + chunk-verified: grep "Set up from the cloud").** Seen live on Izzy's own machine (computer-use): his app resumed straight INTO an active office-scan run — the welcome screen, and with it "Prepare a Yealink for delivery / manage phones", was UNREACHABLE until Cancel setup. That was the whole "I don't see any of those options" mystery; the deploy had landed fine. Fix: the stuck row on the progress list now offers **"Set up from the cloud — no password"** for a found Yealink (needsAttention + mac + managedModels + vendor≈yealink) → opens `ManagedPhonePanel` with **MAC + model prefilled** (`initial` prop; ⛔ the serial NEVER reaches the browser — typed from the label) + a hint about the one hand-reset; the live×4 + done step blocks now yield to the managed panel (`!managedMode`) instead of double-rendering; `serial_number_required` has plain words. The FULL-SCREENS source guard was updated to the new literal (invariant unchanged). Portal tsc 0; wizard test files 29/30 (the 1 fail = the documented pre-existing "standing provisioning listener"). His app reloaded onto this build via the in-app update toast. ⏳ The new stuck-card button has not yet been SEEN on a live stuck card (needs a scan run hitting the password wall; Izzy was mid-work, screen-driving stopped).
