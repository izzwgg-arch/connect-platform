...
- ⛔⛔ **ROUND 3 (09-18 ~03:00Z): Contabo's first reply = the preempted deflection** ("no ping" = our geo firewall; wants a VNC rewrite of `/etc/network/interfaces` + networking restart + ROOT CREDENTIALS — ⛔ never run those steps on the live PBX, never send credentials), **and they power-cycled the PBX TWICE uninvited** (02:28 + 02:31:52 UTC, interfaces rewritten 02:31:23 between; not Izzy; no SSH/console/shutdown trace in the guest = hypervisor-side, only Contabo can). ~4 min down at 0 active calls; platform verified back clean (67 regs, 143 contacts, RTP range intact). Sharpened reply drafted; awaits Izzy's panel login (sessions expired). Handoff §12.
