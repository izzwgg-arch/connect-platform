...
- ⛔⛔ **Phone width: an implicit grid column is sized by its widest child.** `.content`, `.col`, `.list`, the settings/admin layouts and the phone `.app` column were `display: grid` with no template (or a bare `1fr`, which is `minmax(auto, 1fr)`) — an `auto` track takes the min-content of its items, and a `.card` holding a `.tblwrap` (overflow-x: auto) STILL contributes the table's full width, so one wide table pushed the entire page past a 412 px screen (connections, RFQ quotes, privacy matrix; the top bar's account menu went off-screen). Fix is one rule in `globals.css`: every stacking grid is `grid-template-columns: minmax(0, 1fr)`; `.two/.three/.app/.admin/.msg-shell/.settings-grid` phone templates are `minmax(0, 1fr)` too. Any NEW `display: grid` stack must join that list or it re-opens the hole. Sibling rules from the same run: `.in { min-width: 0 }` (Android time inputs cannot shrink), `.tbl .ph-hide` hides secondary columns on phones (connections uses it), `.main` bottom padding 88 px + `scroll-margin` so nothing scrolled to the bottom sits under the fixed tab bar.
- ⛔ **WebKit reports the test's own cancelled fetch as a `pageerror` on some pages and a console error on others** ("…localhost:3101/auth/me due to access control checks."). `e2e/theme-a11y.spec.ts` runs `isNavigationCancel()` on BOTH listeners; filtering only `console` left 2 iPhone failures that looked like CORS.
- ⛔ **E2E runs one browser project at a time, `--workers=1`, against `next start` of a production build** (`NEXT_PUBLIC_COMMUNITY_API_URL=http://localhost:3101 npx next build` with :3100 freed first). Dev mode + 4 workers overwhelmed this box and produced timeouts that were not product bugs. Two cases are timing-flaky once in ~3 runs (Android TOTP enable across a 30 s code window; iPhone "sign out other devices") — both pass on re-run; retry them before reading a failure as a defect.
...
- ✅ **Proof**: 243/243 api integration tests on the real db; Tier-1 loop 17 flows × 20 = 340/340 over HTTP; chaos 5/5; adversarial isolation 5/5; mobile 65/65 + android bundle export; control inventory 786/786; restore drill executed locally (`scripts/community/restore-drill.sh` → OK); load test at 100 VUs found the feed at 58 s p50 → fixed to 0.2–0.5 s and RFQ create 11.5 s → 2.2 s. **Playwright E2E on the final build: desktop-chromium 47 + 1 skipped, mobile-iphone (WebKit) 48/48, mobile-android (Pixel 7) 48/48 — 0 failed**; driving the phone projects found 9 more real defects (listed in the completion report) that the desktop run and every unit suite missed. 10-minute soak sample (100 VUs) recorded in the completion report.
...
- ⏳ **Open (from the completion report):** ~~Next 14.2 advisories~~ (done — 15.5.25, E2E-gated); mobile never run on a physical device/emulator (Playwright device projects only); no malware scanner (uploads marked `not-scanned`); no OTel exporter; the full 1-hour soak still to run on the target server (10-minute local sample recorded); keys (SMTP/Google/Apple/Anthropic/EAS) unset; the Loopcom-side sidebar link is a later portal change.
