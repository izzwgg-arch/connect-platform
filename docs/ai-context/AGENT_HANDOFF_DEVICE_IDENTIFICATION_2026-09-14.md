...
**ROUND 22b — the live retry (same evening).** Izzy held OK on the T42S and pressed "Try again" 14 times (21:27–21:33Z). Desktop log: two `factory_reset … refused:locked` (21:26:40/52) then NOTHING; audits: `DESK_PHONE_RETRY` → ASSIGNED → halt in <1 s, every press, plus `DESK_PHONE_VENDOR_REDIRECT_CONFLICT` each time. Two defects: **(1) the driver memo outlived the retry** — `retryPhone` only POSTed `/retry`; the window's `PhoneMemo` (`locked`, `passwordUnavailable`, `defaultCredentialsTried`) was untouched, so the next advance carried "no password" and the ladder halted without an op. Fix: `createSetupDriver().retried(phoneId)` deletes the memo (keeps `credentialRef`); `DeskPhoneWizard.tsx` calls it after both `/retry` posts. **(2) the RPS-conflict note clobbered the halt note** — `ensureYealinkRedirect` is fire-and-forget from `/retry` with the ASSIGNED row; the ladder halts ~300 ms later; the claim's write lands ~1 s later with the stale snapshot and set `customerNote` = the conflict sentence, erasing "hold OK ~10 s". Fix: `writeOutcome` re-reads `state/haltedReason/customerNote` (`findUnique`, tolerated absent in the fake db) and writes the note only onto an un-halted row whose note is empty or already ours. **The handset itself never reset** (see summary). ⛔ Lesson: a client-side memo that gates the ladder must be cleared by every "fresh go" the server performs, and a fire-and-forget writer must re-read before it writes a customer-facing sentence.
