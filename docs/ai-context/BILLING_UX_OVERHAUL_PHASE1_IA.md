...
| **Phase 3e (2026-05-17)** | **Local vs toll-free DID pricing** | Split **Local phone numbers** + **Toll-free phone numbers** cards; separate invoice lines (`lineItemKind` metadata); `tollFreeNumbers` qty override; `billingTollFreeDidPriceCents`; NANP NPA detection — no schema migration |
| **Phase 3f (2026-05-17)** | **Taxes & fees workspace (Phase A)** | **`AdminTaxesFeesWorkspace`**; `metadata.billingTelecomFees`; estimate + customer-visible preview; cycle/branding → **Invoice & billing** tab; TaxProfile sync for core tax lines |
