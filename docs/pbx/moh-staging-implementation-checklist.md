...
- **Caller-leg hold hook (inbound hold):** the **generic runtime** context `[connect-localdial-moh]` (2026-07-02 hardening; `install-connect-caller-leg-moh.sh` / `extensions__67_connect_localdial_moh.conf`), invoked by one guarded GosubIf inserted after the `U(sub-before-bridging-call` anchor in `[sub-local-dialing]`. Resolves tenant at call time from `${TENANT_PREFIX}` → `pbx_tenant_map/<id>/slug` → `admin_moh_class`→`moh_class`→`active_moh_class`. Covers all current + future tenants with no per-tenant regeneration. See proof doc §P. Staging validation: publish MOH for an **unpublished** test tenant and confirm inbound hold plays the chosen class with **no** installer re-run.
