...
**Status update 2026-07-27:** live call logs (Landau T21 ext 101) proved two
gaps in the original channel-name-only extension detection: (a) this build
names extension endpoints `T<id>_<ext>_<device>`, so the raw channel tail
never matches the published `<ext>` key family; (b) the leg an outside
caller rides (and hears when the extension holds them) is a trunk endpoint
with no tenant token at all. The resolver heredoc now derives an
**effective extension token** — channel token → token sans device suffix
(`CUT(_,1)`) → callee arg → caller arg — adopting a candidate only when its
per-extension key family exists under the context-resolved tenant slug
(key-existence gate; outside numbers can never match). Still repo-only:
not yet installed on the PBX.
