...
- Full suites: desktop 109/109 coworker (371+ whole app), agent coworker 72/72.
- **Security/attack suite 18/18** (`computerControlSecurity.test.ts`) — unknown/malformed tools, a wire claiming `approved:true`, traversal/device/UNC/ADS/junction, encoded+iex+downloaded-exe PowerShell, eight phrasings of reading a secret, every road to one protected file, program tools with no session / another task's session / after it ended, ask-once NOT pre-approving a destructive tool, kill switch mid-session, a live phone call, a password field's value, 500 hostile calls.
- **Real-machine acceptance** (`scripts/coworker-acceptance/run-computer.mjs`, ordinary chat sentences to the deployed agent, verified independently of the model's claims — files on disk, the acceptance app's own state JSON, a separate UIA probe process, the desktop journal): final run **18 PASS / 2 FAIL / 0 BLOCKED**. Across five runs the score was 13/17/17/18/18 — ⛔ the variance is the MODEL's choice of road, not the mechanisms; every capability passed at least once with independent evidence.
- Proven live on DESKTOP-8HUS877 through the installed app: Notepad typed via UIA and read back by a separate probe; File › Save As through the real dialog → file on disk; **Calculator display "16,907" with zero positional clicks**; the acceptance app's text/radio/checkbox/combo/dialog/tab verified from the app's OWN state file; the UIA-invisible purple control clicked after `computer_screen_look` (counter 0→1); Explorer opened but the folder made with the file tool; a protected file refused on every road; an injected instruction summarized not obeyed; `POST /coworker/cancel` → zero actions afterwards; **yield: the model drove the real cursor, a separate process moved the mouse, the next action came back `paused_by_person`**; the Local Worker killed mid-task → detected and restarted.
- Deploy: agent rebuilt on loopcom and container-verified at each step (final `1ed9deda`); desktop `0.1.17-rc.21` built from a clean export of the branch tip, installed, and the installed `app.asar` hash compared byte-for-byte with the build. ⛔ NOT published to the fleet feed.
- NOT proven: **no human has pressed Allow** (a script invokes the real button through UIA and the report says so); Escape pressed by a person; elevated (UAC) PowerShell approved by a person; multi-monitor / DPI scaling (one display); no customer has any of it.
