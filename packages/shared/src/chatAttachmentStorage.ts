 ...
 *
 * Lives in `@connect/shared/chatAttachmentStorage` so both the API (uploads,
 * downloads) and the worker (audio-conversion + re-upload for MMS) can write
 * and read the same files without duplicating the driver logic.
 */
...
/**
 * Read an attachment fully into memory. Convenience wrapper for callers that
 * need the bytes (e.g. the worker's MMS audio converter); uses the same
 * driver-agnostic read path as `readChatAttachment` but materialises the
 * stream into a Buffer.
 */
export async function readChatAttachmentBuffer(storageKey: string): Promise<Buffer | null> {
  const obj = await readChatAttachment(storageKey);
  if (!obj) return null;
  const chunks: Buffer[] = [];
